...
- **Table** — keyed store backed by memcache, so any process reads or writes a value via `Table_Node::table( $ns )` and then `lookup()` / `store()` / `forget()`. Write-through, so it composes mid-graph. An optional third argument puts an in-memory L1 in front, trading bounded staleness for speed.
