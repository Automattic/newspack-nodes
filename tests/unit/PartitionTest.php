...
class PartitionTest extends TestCase {
	...
	public function test_allow_large_writes_throws_if_already_held(): void {
		...
		$p1->arguments( "{$this->tmp}.p0 " . ( 64*1024 ) . " 2 4 86400 0" );
		$p1->name( 'p1' );
		$p1->allow_large_writes();

		$p2 = new Partition_Node();

		$p2->arguments( "{$this->tmp}.p0 " . ( 64*1024 ) . " 2 4 86400 0" );
		$p2->name( 'p2' );
	...
	private function debounced_partition( string $dir, int $debounce_ms ): object {
		...
		$p->arguments( "{$dir} " . ( 64 * 1024 ) . ' 2 4 86400 0' );
		$p->name( 'dbp' );
	...
	public function test_debounced_reacquire_appends_at_the_live_end_after_another_writer(): void {
		...
		$other->arguments( "{$this->tmp}.p0 " . ( 64 * 1024 ) . ' 2 4 86400 0' );
	...
	public function test_read_at_allows_reads_past_ten_megabytes(): void {
		...
		$p = new Partition_Node();
		$p->arguments( "{$this->tmp}.p0 " . ( 16 * 1024 * 1024 ) . " 2 4 86400 0" );
		\mkdir( "{$this->tmp}.p0", 0755, true );
		$size = 11 * 1024 * 1024;
	...
	public function test_read_at_rejects_negative_segment_id(): void {
		$p = new Partition_Node();
		$p->arguments( "{$this->tmp}.p0 " . ( 1024 * 1024 ) . " 2 4 86400 0" );
	...
	public function test_read_tail_index_by_returns_latest_record_per_key(): void {
		...
		$log->arguments( "{$dir} " . ( 64 * 1024 ) . ' 2 2 0 0' );
		foreach (
			[
	...
	/** serialize_record() equivalent: the exact bytes replica mode writes for a message. */
	private function replica_record( array $message ): string {
		return Message::packed( $message ) . "\n";
	}

	/**
	 * Stamp a message's ID with a self-consistent `<seg>:<off>:<len>` crumb whose
	 * declared length equals its own serialized-record length — the fixed-point
	 * invariant a real replication chain maintains (each on-disk record carries its
	 * own crumb, so the length is stable). Iterates because the digit-count of the
	 * length can grow the record by a byte.
	 */
	private function stamp_position( array $message, int $segment, int $offset ): array {
		$len = 0;
		do {
			$message[ Message::ID ] = "{$segment}:{$offset}:{$len}";
			$actual                 = \strlen( $this->replica_record( $message ) );
			$settled                = ( $actual === $len );
			$len                    = $actual;
		} while ( ! $settled );
		return $message;
	...
	public function test_replica_write_lands_at_exact_segment_and_offset(): void {
		$p = new Partition_Node();
		$p->arguments( "{$this->tmp}.p0 " . ( 64 * 1024 ) . " 2 4 0 0" );
		$p->is_replica();

		$msg    = $this->stamp_position( $this->produce( 'hello' ), 0, 0 );
		$record = $this->replica_record( $msg );
		$p->fill( $msg );

		$this->assertSame( $record, \file_get_contents( "{$this->tmp}.p0/0.log" ) );
		$this->assertSame( [ 'hello' ], $this->read_partition_values( $p ) );
	}

	public function test_replica_write_at_end_of_existing_segment_appends(): void {
		$p = new Partition_Node();
		$p->arguments( "{$this->tmp}.p0 " . ( 64 * 1024 ) . " 2 4 0 0" );
		$p->is_replica();

		$first     = $this->stamp_position( $this->produce( 'aaa' ), 0, 0 );
		$rec_first = $this->replica_record( $first );
		$p->fill( $first );

		$second     = $this->stamp_position( $this->produce( 'bbb' ), 0, \strlen( $rec_first ) );
		$rec_second = $this->replica_record( $second );
		$p->fill( $second );

		$this->assertSame( $rec_first . $rec_second, \file_get_contents( "{$this->tmp}.p0/0.log" ) );
		$this->assertSame( [ 'aaa', 'bbb' ], $this->read_partition_values( $p ) );
	}

	public function test_replica_write_rejects_non_boundary_offset(): void {
		$p = new Partition_Node();
		$p->arguments( "{$this->tmp}.p0 " . ( 64 * 1024 ) . " 2 4 0 0" );
		$p->is_replica();

		$p->fill( $this->stamp_position( $this->produce( 'hello' ), 0, 0 ) );

		// Offset 3 sits inside the seed record — byte 2 is not a newline.
		$next = $this->stamp_position( $this->produce( 'boom' ), 0, 3 );

		$this->expectException( \RuntimeException::class );
		$this->expectExceptionMessage( 'message boundary' );
		$p->fill( $next );
	}

	public function test_replica_write_rejects_offset_past_eof(): void {
		$p = new Partition_Node();
		$p->arguments( "{$this->tmp}.p0 " . ( 64 * 1024 ) . " 2 4 0 0" );
		$p->is_replica();

		// Fresh partition: segment 0 is empty, any offset > 0 is past EOF.
		$msg = $this->stamp_position( $this->produce( 'hello' ), 0, 5 );
	...
	public function test_replica_write_rejects_length_mismatch(): void {
		$p = new Partition_Node();
		$p->arguments( "{$this->tmp}.p0 " . ( 64 * 1024 ) . " 2 4 0 0" );
		$p->is_replica();

		$msg              = $this->produce( 'hello' );
		$msg[ Message::ID ] = '0:0:99999';
	...
	public function test_replica_write_rejects_malformed_id(): void {
		$p = new Partition_Node();
		$p->arguments( "{$this->tmp}.p0 " . ( 64 * 1024 ) . " 2 4 0 0" );
		$p->is_replica();

		$msg              = $this->produce( 'hello' );
		$msg[ Message::ID ] = 'not-a-position';

		$this->expectException( \InvalidArgumentException::class );
		$p->fill( $msg );
	}

	public function test_replica_write_mid_file_truncates_tail_and_later_segments(): void {
		$p = new Partition_Node();
		$p->arguments( "{$this->tmp}.p0 " . ( 64 * 1024 ) . " 2 4 0 0" );

		// Append two records into segment 0 the normal way.
		$msg_a = $this->produce( 'aaa' );
		$rec_a = $this->replica_record( $msg_a );
		$p->fill( $msg_a );
		$p->flush();
		$msg_b = $this->produce( 'bbb' );
		$p->fill( $msg_b );
		$p->flush();

		// Fabricate a later segment 1 with a valid record.
		\file_put_contents( "{$this->tmp}.p0/1.log", $this->replica_record( $this->produce( 'later' ) ) );

		// Replica-write a new record at the boundary AFTER record A.
		$p->is_replica();
		$new     = $this->stamp_position( $this->produce( 'new' ), 0, \strlen( $rec_a ) );
		$rec_new = $this->replica_record( $new );
		$p->fill( $new );

		$this->assertFileDoesNotExist( "{$this->tmp}.p0/1.log" );
		$this->assertSame( $rec_a . $rec_new, \file_get_contents( "{$this->tmp}.p0/0.log" ) );
		$this->assertSame( [ 'aaa', 'new' ], $this->read_partition_values( $p ) );
	}

	public function test_is_replica_round_trips_via_dump_config(): void {
		$p = new Partition_Node();
		$p->name( 'firehose' );
		$p->arguments( "{$this->tmp}.p0 " . ( 64 * 1024 ) . " 2 4 0 0" );
		$p->is_replica();
		$this->assertStringContainsString( 'cmd firehose:config is_replica', $p->dump_config() );
	}

	public function test_replica_write_to_a_new_higher_segment_at_offset_zero(): void {
		$p = new Partition_Node();
		$p->arguments( "{$this->tmp}.p0 " . ( 64 * 1024 ) . " 2 4 0 0" );

		// Normal-append a record into seg 0; seg 1 does not exist.
		$seed = $this->produce( 'seed' );
		$p->fill( $seed );
		$p->flush();
		$seg0_before = \file_get_contents( "{$this->tmp}.p0/0.log" );

		// Replica-write a record addressed to the not-yet-existent seg 1 at offset 0.
		$p->is_replica();
		$new    = $this->stamp_position( $this->produce( 'new' ), 1, 0 );
		$record = $this->replica_record( $new );
		$p->fill( $new );

		$this->assertSame( $record, \file_get_contents( "{$this->tmp}.p0/1.log" ), 'record must land in the target higher segment' );
		$this->assertSame( $seg0_before, \file_get_contents( "{$this->tmp}.p0/0.log" ), 'seg 0 must be untouched' );
	}

	public function test_replica_write_rewind_segments_cache_matches_disk(): void {
		$p = new Partition_Node();
		$p->arguments( "{$this->tmp}.p0 " . ( 64 * 1024 ) . " 2 4 0 0" );
		...
		$cache = $this->read_segments_cache( $p );
		$this->assertNotNull( $cache, 'cache stays warm after a replica rewind' );
		$this->assertSame( $p->get_segments( true ), $cache, 'segments_cache equals the on-disk survivor list after rewind' );
		$this->assertSame( [ 0 ], \array_column( $cache, 'id' ), 'cache holds only the surviving segment' );
	}

	public function test_replica_rewind_truncates_stale_idx_lines(): void {
		$p = new Partition_Node();
		$p->arguments( "{$this->tmp}.p0 " . ( 64 * 1024 ) . " 2 4 0 0" );
		// One .idx line per record: the record's VALUE.
		$p->with_index( fn ( array $message, array $pos ) => (string) $message[ Message::VALUE ] );
		...
		// Rewind past record A and overwrite record B; the stale 'bbb' .idx line must go.
		$p->is_replica();
		$new = $this->stamp_position( $this->produce( 'new' ), 0, \strlen( $rec_a ) );
		$p->fill( $new );

		$idx_lines = \array_values( \array_filter( \explode( "\n", (string) \file_get_contents( "{$this->tmp}.p0/0.idx" ) ), static fn ( $l ) => '' !== $l ) );
		$this->assertSame( [ 'aaa', 'new' ], $idx_lines, '.idx must be rewound alongside the .log (no stale bbb line)' );
	}

	public function test_replica_write_tracks_largest_msg_sent(): void {
		$p = new Partition_Node();
		$p->arguments( "{$this->tmp}.p0 " . ( 64 * 1024 ) . " 2 4 0 0" );
		$p->is_replica();

		$msg    = $this->stamp_position( $this->produce( \str_repeat( 'x', 100 ) ), 0, 0 );
		$record = $this->replica_record( $msg );
		$p->fill( $msg );

		$this->assertSame( \strlen( $record ), $p->largest_msg_sent() );
	}
}
