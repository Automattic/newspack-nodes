...
**Empty-string short-circuit in `arguments()` overrides.** The base `arguments($args)` returns early on `'' === $args` without walking the schema (no token-driven assignment). A subclass that overrides `arguments()` to do post-assignment derivation (e.g. `Partition_Node` re-deriving `partition_dir = "{$base_dir}/p{$partition}"`) MUST mirror that early-return when `$args === ''` — otherwise re-deriving from declaration-default props yields filesystem-root junk like `/p0`. Partition_Node is the reference template.
