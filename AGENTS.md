...
| `includes/class-lru-cache.php` | `LRU_Cache` — the bucket LRU from Tachikoma's `Table.pm`, rotating on capacity AND on an absolute wall-clock grid so a restarted process keeps its predecessor's phase. Promotion (a hit moves to the newest bucket) is the default a WORKING SET wants; `without_promotion()` is what a read-through tier wants, and `Table_Node`'s L1 uses it |
