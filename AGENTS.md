...
| `includes/class-topic-probe-node.php`, `includes/class-probe-record.php` | `Topic_Probe_Node` — periodic per-worker Consumer-stats sweep (port of Tachikoma TopicProbe.pm, consumer branch). `Probe_Record` fixes the positional layout of a `topicprobe.p0` VALUE, parity-pinned to `src/runtime/probe-record.js`. Stock `topologies/topic-probe.tsl` |
| `includes/class-job-probe-node.php`, `includes/class-jobstats-record.php` | `Job_Probe_Node` — the jobs analog: one snapshot per job IDENTITY per tick into the shared `jobstats` log (a Job_Worker owns many identities, so one worker yields many records). `Jobstats_Record` fields 2..7 are the work done since that identity's previous sweep, with an `ELAPSED_MS` covering it, so a reader divides ONE record and a ~595s worker recycle is just another window rather than a counter reset. Parity-pinned to `src/runtime/jobstats-record.js` |
