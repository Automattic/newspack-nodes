...
- **`Log` rotates `/var/log` style and `prune_rotated()` reserves BOTH the `{filename}.N` and legacy `{filename}-` prefixes.** `rotate()` shifts `{filename}.N` → `{filename}.(N+1)` (highest index first), renames the current file to `{filename}.0`, and reopens fresh. `prune_rotated()` globs both `{filename}.[0-9]*` (current scheme) and `{filename}-*` (legacy date-named scheme, kept so pre-existing rotations age out by mtime) and keeps the `max_rotations` newest by mtime. Storing unrelated files under either prefix (e.g. `out.log.keep`, `out.log-keep_forever`) makes them eligible for pruning. Don't co-locate other artifacts.
