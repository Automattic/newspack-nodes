...
| `includes/class-log-sources.php` | `Log_Sources` — the fixed name→log-source registry the `/log/stream` SSE controller and the `taillog` verb (`Log_Sources::taillog()`, moved off the interpreter) both consume |
...
| `includes/class-{remote-link,remote-source}-node.php`, `includes/class-remote-settings-migration.php` | Remote "be the browser" SSE+HTTP channels: `Remote_Link_Node` patrons an `SSE_In` + `HTTP_Out` sibling pair (heartbeat/reconnect/status); `Remote_Source_Node` extends it + `use`s `Durable_Reader` for durable SSE-pull aggregation. `Remote_Settings_Migration` is a one-time option-rename guard |
...
- **Tutorial track** (the flattened `docs/README.md` order — start-here bucket, then production): `docs/getting-started.md` → `docs/writing-a-plugin.md` → `docs/writing-a-dashboard.md` → `docs/writing-a-real-plugin.md` → `docs/writing-a-real-dashboard.md` → `docs/writing-a-view-node.md`
