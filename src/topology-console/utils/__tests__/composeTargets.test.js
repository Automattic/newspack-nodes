...
describe( 'buildComposeTargets', () => {
	it( 'puts _command_interpreter first, then each node id and its :config sidecar, sorted', () => {
		const nodes = [ { id: 'echo' }, { id: 'tee_a' } ];
	...
	it( 'de-dupes and drops a redundant _command_interpreter entry from nodes', () => {
		const nodes = [ { id: names.COMMAND_INTERPRETER }, { id: 'echo' } ];
		expect( buildComposeTargets( nodes ) ).toEqual( [
			names.COMMAND_INTERPRETER,
			'echo',
			'echo:config',
