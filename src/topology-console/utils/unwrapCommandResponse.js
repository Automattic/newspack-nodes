 ...
 *
 * The substrate's CommandInterpreter wraps every verb response this way
 * (see `CommandInterpreter::interpret()` in class-command-interpreter.php).
 * So every dashboard that calls a verb needs the same double-parse:
 *   - JSON.parse the outer VALUE → { name, payload }
 *   - JSON.parse the inner payload → the actual data
 *
 * Mirrors the application-side helper at
 * `newspack-event-logger-nodes/src/shared/utils/unwrapCommandResponse.js`
 * — the substrate has no dependency on the app, so we keep a local copy
 * rather than reach across plugins.
 *
 * @param {Array} message Seven-field Message tuple as returned by
 *                        CommandClient.send().
 * @return {*} The parsed payload (typically an object or array). Returns
 *             null when the verb returned an empty payload.
 * @throws {Error} If TYPE has TM_ERROR set, throws with the payload string
 *                 as the message. Also throws on malformed input.
 */
