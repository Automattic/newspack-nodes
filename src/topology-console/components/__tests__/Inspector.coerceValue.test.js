...
describe( 'coerceValue', () => {
	...
	it( 'int: pure-numeric strings convert to numbers', () => {
		expect( coerceValue( 'int', '42' ) ).toBe( 42 );
		expect( coerceValue( 'int', '-7' ) ).toBe( -7 );
	} );
	...
	it( 'int: partial typing of a substitution token is preserved', () => {
		...
		expect( coerceValue( 'int', '<' ) ).toBe( '<' );
		expect( coerceValue( 'int', '<p' ) ).toBe( '<p' );
		expect( coerceValue( 'int', '<partition' ) ).toBe( '<partition' );
	} );
	...
	it( 'int: lossy numeric prefixes pass through as strings (not silently truncated)', () => {
		...
		expect( coerceValue( 'int', '123abc' ) ).toBe( '123abc' );
	} );

	it( 'float: pure-numeric strings convert to numbers', () => {
		expect( coerceValue( 'float', '1.5' ) ).toBe( 1.5 );
		expect( coerceValue( 'float', '-0.25' ) ).toBe( -0.25 );
		expect( coerceValue( 'float', '1e3' ) ).toBe( 1000 );
	} );

	it( 'float: TSL substitution tokens pass through as strings', () => {
		expect( coerceValue( 'float', '<config:max_lifetime>' ) ).toBe(
			'<config:max_lifetime>'
		);
	} );
