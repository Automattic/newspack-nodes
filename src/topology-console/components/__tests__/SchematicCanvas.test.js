...
describe( 'SchematicCanvas', () => {
	...
	it( 'backgroundClickAutofitsOnly: a bg-click autofits, never deselects or consumes (transcript/inspector stay)', () => {
		const onDeselect = jest.fn();
		const onViewportChange = jest.fn();
		const onBackgroundClickConsumed = jest.fn( () => true );
		const { container } = render(
		...
		expect( onViewportChange ).toHaveBeenCalled();
		expect( onDeselect ).not.toHaveBeenCalled();
		expect( onBackgroundClickConsumed ).not.toHaveBeenCalled();
	} );
