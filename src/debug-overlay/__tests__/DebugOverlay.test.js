...
function submitRepl( body, line ) {
	const input = body.querySelector( '.topology-repl__input' );
	act( () => {
...
describe( 'DebugOverlay', () => {
	...
	it( 'renders nothing when debug is disabled', () => {
		...
		expect( document.querySelector( '.nodes-debug' ) ).toBeNull();
	} );
	...
	it( 'shows a toggle FAB when enabled, and opens the panel on click', () => {
		...
		const {
			...
			getByRole,
			queryByTestId,
		} = render( <DebugOverlay search="?nodes-debug=1" /> );
		const overlay = body.querySelector( '.nodes-debug' );
		const fab = getByRole( 'button', { name: /debug/i } );
		// The portal host is the provider; the overlay carries none of its own.
		expect( overlay.className ).toBe( 'nodes-debug' );
		expect( fab.closest( '.newspack-nodes-ui' ) ).toBe(
			overlay.parentElement
		);
		expect(
			body.querySelectorAll( '.newspack-nodes-skin-root' )
		).toHaveLength( 1 );
		expect( body.querySelectorAll( '.newspack-nodes-ui' ) ).toHaveLength(
			1
		);
		...
		).toBe( overlay.parentElement );
	} );

	// @longform The overlay is developer chrome over whatever is on screen, and
	// a dashboard shell is `position: fixed; z-index: 99`, so its own 999999
	// ranked only against that shell's children — under any dialog portalled
	// to the body at 100000. It leaves the tree the way the dialogs do.
	it( 'draws at body level, above the dialogs it has to cover', () => {
		mountExospine();
		...
		expect( container.querySelector( '.nodes-debug' ) ).toBeNull();
		const overlay = document.querySelector( '.nodes-debug' );
		expect( overlay ).not.toBeNull();
		expect(
			overlay.closest( '.newspack-nodes-skin-root' ).parentElement
		).toBe( document.body );
	} );

	it( 'leaves its host tree and takes exactly one provider with it', () => {
		mountExospine();
		const {
			baseElement: body,
			getByRole,
		...
		expect( overlay.className ).toBe( 'nodes-debug' );
		// Out of the host entirely, under the portal's own provider.
		expect( host.contains( overlay ) ).toBe( false );
		expect( fab.closest( '.newspack-nodes-ui' ) ).toBe(
			overlay.parentElement
		);
		// One provider inside the overlay's own tree, never a second nested
		// under it — the host keeps its own, out here.
		expect(
			overlay.querySelectorAll( '.newspack-nodes-skin-root' )
		).toHaveLength( 0 );
		fireEvent.click( fab );
		expect( queryByTestId( 'debug-panel' ) ).not.toBeNull();
		expect(
	...
	it( 'set_skin REPL builtin applies the selected theme class to the overlay shell', () => {
		...
		submitRepl( body, 'set_skin blueprint' );
		expect(
	...
	it( 'mounts the topology Header above the themed app shell', () => {
		...
		expect(
			body.querySelector( '.topology-header .topology-brand' )
		).not.toBeNull();
		// Hidden in view mode: Open/Save/Delete/New + EDIT (canEdit=false).
		expect( body.querySelector( '.topology-mode__btn--open' ) ).toBeNull();
		expect( body.querySelector( '.topology-mode__btn--save' ) ).toBeNull();
		expect( body.querySelector( '.topology-mode__btn--new' ) ).toBeNull();
		expect(
			body.querySelector( '.topology-mode__btn--delete' )
		).toBeNull();
		// In the overlay, the LIVE button is replaced by an X close button.
		expect(
			body.querySelector( '.topology-mode__btn--close' )
		).not.toBeNull();
		expect( body.querySelector( '.topology-mode__btn--live' ) ).toBeNull();
	} );
	...
	it( 'closing via Header X close button hides the panel', () => {
		...
		fireEvent.click( getByRole( 'button', { name: /debug/i } ) );
		expect( queryByTestId( 'debug-panel' ) ).not.toBeNull();
		fireEvent.click( body.querySelector( '.topology-mode__btn--close' ) );
		expect( queryByTestId( 'debug-panel' ) ).toBeNull();
	} );
	...
	it( 'falls back to the default theme when localStorage.getItem throws', () => {
		...
		try {
			...
			expect(
				body.querySelector( '.topology-app.is-palette-collapsed' )
			).not.toBeNull();
		} finally {
	...
	it( 'palette starts collapsed by default; the toggle expands and persists', () => {
		...
		const toggle = body.querySelector( '.topology-palette__toggle' );
	...
	it( 'set_skin rejects an unknown skin name and leaves the theme unchanged', () => {
		...
		submitRepl( body, 'set_skin not-a-theme' );
		expect(
			document.documentElement.classList.contains( 'theme-newspack' )
		).toBe( true );
		expect(
	...
	it( 'header double-click on background fires toggleMaximize (frame expands)', () => {
		...
		fireEvent.click( getByRole( 'button', { name: /debug/i } ) );
		const headerDrag = body.querySelector( '.nodes-debug__header-drag' );
		expect( headerDrag ).not.toBeNull();
		const panel = body.querySelector( '.nodes-debug__panel' );
	...
	it( 'header double-click on a control (button) does NOT maximize (skip path)', () => {
		...
		const headerBtn = body.querySelector( '.topology-mode__btn' );
	...
	it( 'shows resize handles around the panel when open', () => {
		...
		const handles = body.querySelectorAll( '.nodes-debug__resize' );
	...
	it( 'offers `_http` as the only cwd, not plumbing or an arbitrary `_` node', () => {
		...
		const selects = body.querySelectorAll( '.topology-select' );
	...
	it( 'inspector action through GraphView pops the transcript footer (setReplExpanded=true)', async () => {
		...
		const nodeEls = body.querySelectorAll( '.topology-node' );
		...
		const footer = body.querySelector( '.topology-repl.is-expanded' );
