...
- **`topologies` CI gains `activate` / `deactivate` verbs — the first CI writers of the `newspack_nodes_topologies` active-set option.** `activate <name>` materializes the current effective active set (`Bootstrap::get_topologies()`), adds the name, persists the option, invalidates the config cache (the same `Config::invalidate_options_cache()` + `Config::reset()` pair, in the same order, `Supervisor::check_config()` uses so the next `expand_workers()` sees the write), then spawns the fleet immediately via `Supervisor::spawn_fleet()`; it returns `{name, active:true, spawned:<int>}` and refuses an unknown name with `error: unknown topology '<name>'` before any write. `deactivate <name>` is the symmetric drain: remove the name, persist, invalidate, then `Supervisor::kill_readers([name])`, returning `{name, active:false}`. Both require `manage_options`.
