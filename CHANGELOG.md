...
- **`Partition_Node::read_latest_value_at( $offset_dir )` — read the newest offsetlog record's VALUE.** The walk to read the latest committed checkpoint (`get_segments(true)` → newest segment → `read_at` → last non-empty line → `Message::unpacked` → VALUE, guarded + try/caught) was duplicated in `CLI::read_offsetlog_entry()` and the `newspack-ai-newsletter` example's `Insights_CI::read_cache_items()`. Both now delegate to this one public reader; `CLI::read_offsetlog_entry()` becomes a one-line wrapper and the example keeps only its `cache.items` extraction. No behavior change (the example gains the reader's explicit `'' === $bytes` guard, which was already a no-op there via the empty-lines path).
