...
- **The debug overlay no longer drops REPL/gesture commands on a fast open-and-type.** The overlay binds `shell.sink` to the page's `_command_interpreter` via a render-effect; opening the panel and typing immediately after page load — before the dashboard's `mountExospine` effect registered the interpreter — left `shell.sink` null, so `ls` / `dump_node` / connect / etc. were silently dropped (the REPL surfaced a `shell.sink is null` canary). Both dispatch paths — the REPL (`useDebugRepl`) and the GUI gestures (`useDebugGraph`'s `sendVerb`) — now resolve the interpreter from `Core` at dispatch time, eliminating the binding race; the stderr canary now fires only when the page genuinely has no interpreter. Adds regression tests for both paths.
