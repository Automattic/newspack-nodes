...
- **Worker Status now renders one foldable node/log tree section per topology instead of a flat global pipeline.** The dashboard groups workers by `type` into `TopologySection`s (header with per-partition status/uptime/heartbeat pills + ALL RUN/ALL DEAD badge + fleet restart, followed by the topology's `TreeEntity` tree); the flat `pipeline-flow` render (with `WorkerConnector` + `LogSection` and the cross-type topo-sort) is removed. Behavior change: a log only renders as a child of a producer/consumer node within its topology, so an orphan catalog log (one with no producer/consumer worker, including the empty-workers case) is no longer rendered on its own. Fold state is owned by `WorkerStatus`. The supervisor card, header chrome (title, total W/R rates, refresh select), connection banner, and loading gate are unchanged.
