...
- **`workers` CI gains a `purge_orphans` verb — the manager-facing trigger for the shipped housekeeping GC.** Runs `Log_Cleaner::cleanup_orphan_partitions( <base_dir> )` (the same liveness-free sweep that deletes first-level `logs/*` + `offsets/*` dirs not in the resolved declared set), then projects the removed absolute paths to basenames and returns `{ removed: <string[]>, count: <int> }`. Thin by design — all keep/delete logic lives in `Log_Cleaner`; the verb only fires the sweep and shapes the result. Requires `manage_options`. Backs the Topology Manager's "Purge orphans" button.
...
- **Topology Manager "Purge orphans" toolbar action.** A toolbar above the topology list with a "Purge orphans" button that dispatches the `workers` CI `purge_orphans` verb (no args, via the same interpreter + PendingReplies path the per-topology `restart` uses) to run the housekeeping GC, then surfaces a transient inline result: `Removed N orphan dir(s): <basenames>` when something was reaped, `Nothing to purge` when not, and a small inline error (caught, so a rejected purge can't crash the render) on failure. `useTopologyManager` exposes the new `purgeOrphans()` mutation resolving with the verb's `{ removed, count }` payload.
...
- **Topology Manager / worker-status tree polish — one consolidated head, flatter logs, console-linked names.** Eight changes to the hub tab and its embedded tree: (1) a node's per-partition read-status pills (`P0 R 0 B/s`, `P1 …`) lay out on ONE horizontal flex row (`.tree-node-row`, wrap-on-overflow) instead of stacking. (2) A LEAF log group (segment rows only, no downstream subtree) flattens — its group-label header is dropped and each partition row is labelled with the CONCRETE name (`jobs.p0`, `jobs.p1`) instead of bare `P{n}`; a SOURCE log that feeds a subtree keeps its group label + caret and renders the subtree once (no per-partition duplication). (3) Flattened leaf logs lose their fold caret. (4) A small vertical gap (`.tree-branch--flat-log`) replaces the removed label/caret. (5) `displayName` no longer Title-Cases node slugs — the tree is now 1:1 with the console (`job-router`, not `Job Router`). (6) Each topology's heading name is a link to the console scoped to that topology (`admin.php?page=newspack-nodes-topology&topology=<name>`). (7) The fleet-restart button keeps its heading placement but uses the larger `↻` icon. (8) `TopologySection`'s own header is removed entirely (it's tree-only now); the per-partition process pills (`.topology-partition` — P{n} + uptime + heartbeat + restart_pending) and the ALL RUN / ALL DEAD badge move up into the manager card heading, computed from the topology's `status.workers` via `partitionSummaries`. worker-status.scss (orphaned when the standalone Worker Status dashboard was deleted) is now `@use`d by `TopologyManager.scss` so the embedded tree and hoisted pills are styled.
