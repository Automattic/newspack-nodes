...
- **Topology `save` / `delete` now arm the housekeeping GC.** Both verb handlers in `Topologies_CI_Node` set `Log_Cleaner::LOGS_DIRTY_OPTION = '1'` before returning, so the next supervisor sweep (which already calls `Log_Cleaner::cleanup_orphan_partitions()` every ~15s) cleans up offsetlog/ipc dirs orphaned by a topology edit that drops a source. No new scheduling.
