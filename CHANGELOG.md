...
- Wrapping the dashboard UI strings for translation (`@wordpress/i18n` `__()`/`_n()`/`sprintf`, domain `newspack-nodes`); the Tier-1 dashboards are done — `WorkerStatus`, `TopologyConsole`, and the node `Inspector`. `ConnectionBanner` consumers pass a translated `message` (its default stays an English fallback); protocol/command strings, comparison values, and map-keys stay raw.
