...
- **The canvas measures itself via `ResizeObserver` and the viewport cull gained more LOD tiers, so a multi-thousand-node graph stays light at every zoom.** Nodes are culled to the viewBox plus a half-viewport overscan band on each axis (`NODE_OVERSCAN`), so panning scrolls smoothly and a narrow column doesn't blink out when nudged sideways (the strict no-margin cull dropped a node the instant it crossed the edge). An edge is culled only when BOTH endpoints are off-screen (`isEdgeVisible`); one visible endpoint is enough, so an edge to/from an in-view node always draws even when its peer scrolled off. Below a readable scale (`EDGE_MIN_SCALE`) the entire edge layer is skipped — you can't trace an edge in the whole-graph overview anyway, and that drops ~2.8k paths. Each bare node also has a minimum on-screen size (`MIN_NODE_PX`): at extreme zoom-out the rect is enlarged in world units so it never shrinks to a sub-pixel that some browsers (Firefox) drop. The canvas size feeding the cull comes from a `ResizeObserver` on the SVG rather than a one-shot mount read, so a panel resize re-fits and re-culls instead of measuring against a stale size.
