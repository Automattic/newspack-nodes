...
- **Topology `save` / `delete` now arm the housekeeping GC.** Both verb handlers in `Topologies_CI_Node` set `Log_Cleaner::LOGS_DIRTY_OPTION = '1'` before returning, so the next supervisor sweep (which already calls `Log_Cleaner::cleanup_orphan_partitions()` every ~15s) cleans up offsetlog dirs orphaned by a topology edit that drops a source. No new scheduling. (ipc dirs are reaped by the supervisor's own `cleanup_orphan_ipc()`, which runs unconditionally every config-check tick — see Changed.)
...
- **Orphan `ipc/` dir GC moved from `Log_Cleaner` to the `Supervisor` tick.** The ipc-dir sweep (reaping `ipc/{type}.p{N}` dirs for workers no longer in the fleet) is worker-lifecycle, not log retention, so it left `Log_Cleaner::cleanup_orphan_partitions()` and became `Supervisor::cleanup_orphan_ipc()`, called every config-check tick (~15s) right after the log/offset sweep. It reuses the supervisor's already-built `worker_locks` fleet instead of re-enumerating workers via `Bootstrap::expand_workers()` + `CLI::ls_workers()`, and is decoupled from `LOGS_DIRTY_OPTION` (the dirty flag still gates the log/offset sweep only). A live worker's own `locks/{name}.lock.d` defers its ipc reap; an unrelated worker's lock does not. `Log_Cleaner`'s log, offsetlog, and partition sweeps are unchanged.
...
- **IPC dirs for topologies/partitions no longer active are now garbage-collected.** Nothing swept `ipc/`, so an `ipc/<type>.pN` dir for a removed topology (e.g. `ipc/firehose-workers-and-jobs.p0`) lingered forever. `Log_Cleaner::cleanup_orphan_partitions()` now purges any `ipc/*.pN` dir not in the active `{type}.p{N}` set (`Log_Cleaner::active_descriptors()`), gated on the dir's own descriptor lock (`{type}.pN.lock.d`) so a same-type straggler defers it but unrelated workers don't. Legacy `ipc/<x>/locks` cruft (no `.pN` suffix) is skipped.
- **Offsetlog dirs orphaned by a source being dropped from a topology are now garbage-collected.** When a Consumer's source was removed from a topology, its `offsets/<name>.pN` dir leaked forever — the cleaner only ever swept `offsets/` by partition *number* (on fleet shrink), never by *name*, so it surfaced as a phantom consumer (e.g. `digest / summarized / -`). `Log_Cleaner::cleanup_orphan_partitions()` now also purges any `offsets/*.pN` dir whose basename isn't in the active set, derived by parsing each active topology's TSL (`Topology_Registry::offset_basenames_for()` over `Log_Cleaner::active_descriptors()`). Gated on the orphan dir's own descriptor lock (not partition-wide, so an unrelated worker at the same partition can't perpetually defer the purge) and guarded so a not-yet-loaded app can't wipe live offsetlogs; arbitrary offsetlog names are handled (the match is by declared basename, not a string-mapped log name).
