...
- **Browser "Reset Graph" now restores the dashboard's default wiring deterministically, not just removes user-added nodes.** `mountExospine( build )` takes a build callback, snapshots `Core.nodes` around it, and returns `reinit()` (tear the build-registered soft nodes down — running the build's returned cleanup first — then rebuild them fresh, leaving the backbone and any sibling-registered overlay nodes intact). For a build-delegated graph it also stashes the handle on the per-page `Core` (`Core.reinit` + the managed name set `Core.reinitNames`, both cleared on teardown), so `DebugOverlay` reads `Core.reinit` the way it already reads `Core.node('_command_interpreter')` — no page threads a `reinit` prop. Reset Graph = `reinit()` then drop every node outside the keep-set (`reserved names ∪ Core.reinitNames`); the old first-open `setTimeout` baseline snapshot is gone, so the keep-set is deterministic. A bare `mountExospine()` (the topology console, which keeps its own `resetKey` rebuild) leaves `Core.reinit` null, so the overlay chip stays hidden rather than offering a destructive no-op. The Raw Logs and Worker Status dashboard hooks (`useRawLogsGraph` / `useWorkerStatusGraph`) build through `mountExospine( build )` and bump a render counter on each (re)build so a consumer's `useNodeState` re-subscribes to the freshly-rebuilt view node. A canvas connection rewire dirties the layout (`useDebugLayout` gained `markDirty()`), surfacing both the Reset Layout and (local-scope) Reset Graph chips.
