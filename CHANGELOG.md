...
- **IPC dirs for topologies/partitions no longer active are now garbage-collected.** Nothing swept `ipc/`, so an `ipc/<type>.pN` dir for a removed topology (e.g. `ipc/firehose-workers-and-jobs.p0`) lingered forever. `Log_Cleaner::cleanup_orphan_partitions()` now purges any `ipc/*.pN` dir not in the active `{type}.p{N}` set (`Log_Cleaner::active_descriptors()`), lock-gated per partition. Legacy `ipc/<x>/locks` cruft (no `.pN` suffix) is skipped.
- **Offsetlog dirs orphaned by a source being dropped from a topology are now garbage-collected.** When a Consumer's source was removed from a topology, its `offsets/<name>.pN` dir leaked forever — the cleaner only ever swept `offsets/` by partition *number* (on fleet shrink), never by *name*, so it surfaced as a phantom consumer (e.g. `digest / summarized / -`). `Log_Cleaner::cleanup_orphan_partitions()` now also purges any `offsets/*.pN` dir whose basename isn't in the active set, derived by parsing each active topology's TSL (`Topology_Registry::offset_basenames_for()` over `Log_Cleaner::active_descriptors()`). Lock-gated per partition and guarded so a not-yet-loaded app can't wipe live offsetlogs; arbitrary offsetlog names are handled (the match is by declared basename, not a string-mapped log name).
