...
- **`Log` file-sinks now render as LOG entities (blue, with segment bars) in the Worker Status topology tree.** A `make_node Log <name> <path> <mode> <max_size> <max_rotations>` sink previously surfaced as a white logic node. Three layers: (A) `Topology_Registry::graph_for()` maps the `Log` class to kind `'log'`, emitting `writes` (file basename), `path` (raw path token), `max_size`, and `max_rotations` so the dump can stat the files; (B) `Workers_CI_Node`'s `dump_graph` enumerates each active topology's `Log` sinks, resolves any `<config:KEY>` in the path, stats the current file + its `.N`/legacy `-` rotations, and appends a `logs` catalog entry per sink (`segment_size` = `max_size`, current file = newest/highest segment id, never clobbering a real segmented-log entry); (C) the dashboard's `collapseGraph` treats kind `'log'` like partition/topic — the node collapses to a log vertex named by its file basename, rendered blue with segment bars from the catalog entry.
