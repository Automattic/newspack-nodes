...
- **`Log_Node` rotates `/var/log` (logrotate) style instead of using a date-string suffix.** `rotate()` now shifts each existing `{filename}.N` up to `{filename}.(N+1)` (highest index first, so nothing clobbers), renames the current file to `{filename}.0` (newest rotation), and reopens `{filename}` fresh. The `ROTATED` state's `rotated_to` is now `{filename}.0`. `prune_rotated()` globs BOTH the new `{filename}.[0-9]*` scheme AND the legacy `{filename}-{date}` scheme and keeps the `max_rotations` newest by mtime — so pre-existing date-named rotations age out by mtime alongside the new numeric ones (no migration; they simply disappear over time). `max_rotations <= 0` still disables pruning. `Log_Node` is the only consumer of the rotated-name format; no reader/discovery code parsed it.
