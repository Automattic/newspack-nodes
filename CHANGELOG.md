...
- **The canvas measures itself via `ResizeObserver` and the viewport cull gained two more LOD tiers, so a multi-thousand-node graph stays light at every zoom.** Culling is now strict — any node whose card doesn't intersect the viewBox is dropped, not just its text — and edges gate on a hub-degree LOD (`isEdgeVisible`: an edge with one endpoint on-screen only draws when that endpoint is low-degree, so a 500-edge hub doesn't flood the canvas with connectors to its off-screen neighbours). Below a readable scale (`EDGE_MIN_SCALE`) the entire edge layer is skipped — you can't trace an edge in the whole-graph overview anyway, and that drops ~2.8k paths. The canvas size feeding the cull comes from a `ResizeObserver` on the SVG rather than a one-shot mount read, so a panel resize re-fits and re-culls instead of measuring against a stale size.
