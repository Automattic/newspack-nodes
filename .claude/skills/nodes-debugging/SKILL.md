...
- **Dead letters.** `dl_list [limit]` lists quarantined records newest first, fifty at a time by default, each with its reason, attempt count, first-crash and quarantine timestamps, source breadcrumb and `segment:offset:length` locator. `dl_show <locator>` decodes one read-only. `dl_requeue <locator>` redelivers it to the node's sink, leaving the queued copy in place. `dl_purge` deletes every dead-letter segment and its `.idx`. A reader constructed with an empty `deadletter_dir` has no queue at all: `dl_list` answers with an empty page, and the other three refuse with a TM_ERROR reading `no dead-letter queue configured`. `total` counts the INDEXED records alone. A segment quarantined by a writer that wrote no `.idx` reaches neither `rows` nor `total`, and surfaces only as `unindexed_segments`, the third field on every page — the count the console's Triage panel renders as `N older records predate indexing`. Nothing addresses those records, so `dl_show` and `dl_requeue` cannot reach them: replay the whole segment through `wp nodes ingest`, or let the count rotation age it out.
- **Time travel.** `pause` stops the poll timer and holds the cursor; `step` consumes exactly one record, whether it is forwarded, dead-lettered or refused, and replies with the `{seg, off, at_eof}` cursor; `play` restores the pre-step line mode and resumes; `seek_frame <segment>` jumps to an offsetlog keyframe and restores the state co-committed there, staying paused. `Remote_Source_Node` answers three of them differently, because its `SSE_In` patron is fed by the event loop rather than read a block at a time. `step` consumes one record straight from the pump buffer when one is there; with none, it reconnects from the cursor, takes the next record the spoke sends, then drops the stream again, so its reply carries the starting cursor and the move shows on the next metadata poll, and each click made before the pull answers is owed a record. `pause` disconnects the live stream, keeping what it left buffered for the next step, and a paused source neither ticks, reconnects nor checkpoints unless a step is owed. Any reconnect asks for the record after the last one buffered, so buffered records drain once. `play` re-arms the channel tick, which reconnects from the cursor. After a bare `start`, `recent` or `end` seek, the spoke's `connected` handshake names in CURSORS where the new stream begins, and the source adopts that position, or the first record's breadcrumb from a spoke that sends none; any other handshake is ignored, since it only echoes the position the connect asked for. A seek splits two ways — an explicit `{segment, offset}` reseeds `SSE_In` here, while a bare `start`, `recent` or `end` sentinel is FORWARDED to the spoke, which holds the segments those words name — and either path drops the undrained pump buffer along with the position it belonged to.

Lag is the exception to the pattern: there is no verb for it, and a reader answers no requests at all. Read it from `wp nodes status`, whose `Behind` column is the probe record's `distance` — `Consumer_Node::probe_stats()` calling the `compute_lag()` seam, the same seam `idle_since()` reads. That payload carries more than the distance: `bytes_behind`, `segments_behind` and `caught_up`, then `end_segment` and `end_size` for the newest segment, `end_bytes` for the partition footprint summed over every live segment — which FALLS when retention deletes one — and the `cursor_segment` and `cursor_offset` the distance was measured from, so a distance never travels without its cursor. Only `distance` reaches the CLI column, so read the probe log for the rest.
