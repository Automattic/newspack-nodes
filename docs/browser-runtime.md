...
The stream is the expensive channel. It holds one php-fpm child, one of the site's fixed pool of PHP processes, for its whole life. Past the pool the platform queues each request for a worker and refuses it with 429 when none frees in time, so readers get pages and refusals by turns for as long as the pool stays full. The slot pool caps streams before the platform steps in.
