...
![The correspondence chart: each upstream Tachikoma file or symbol on the left, the port's name on the right, in four groups — Message, Node and Tachikoma.pm; Router, loop and lifecycle; Shell, interpreter and signing; and Nodes/. Blue rows are renames, among them STREAM to KEY, PAYLOAD to VALUE, owner to target, AgeSieve to Age_Sieve_Node, PayloadTimeout to Value_Timeout_Node, Tail.pm to File_Tail_Node, JobSpawnTimer to Connect_Queue_Timer_Node, list_fds to list_handles and connect_sink to set_sink. A red edge marks a deliberate divergence argued below; a dash marks a name with no counterpart, such as edge, answer() and cancel(), Broker.pm, MemorySieve.pm and the forked Job upstream, or Tail_Node, Tap_Node, Graphite_Node and the worker lifecycle here.](img/tl-name-map.png)
...
![Tachikoma's Message.pm beside ours in three bands. The slots: upstream TYPE, FROM, TO, ID, STREAM, TIMESTAMP, PAYLOAD and IS_UNTHAWED at 0 to 7; ours TYPE, TIMESTAMP, FROM, TO, ID, KEY, VALUE and LOCAL, with arrows showing TIMESTAMP moving from 5 to 1 and STREAM and PAYLOAD renamed KEY and VALUE. The flags: fifteen bits from 1 to 16384, BYTESTREAM, EOF, PING, COMMAND, ERROR and INFO shared, bits 16 through 1024 carrying different flags (RESPONSE, PERSIST, STORABLE, COMPLETION and BATCH upstream; STRUCT, REQUEST, RESPONSE, NOREPLY and UNTYPED here), and the four high bits empty here. The wire: upstream packs 'xxxx N n/a* n/a* n/a* n/a* N a*', a length and binary fields; ours is packed(), a JSON list of exactly seven.](img/tl-message-side-by-side.png)

**Why:** TIMESTAMP moves to index 1 so that WHAT and WHEN sit together at the front of the array. STREAM and PAYLOAD become KEY and VALUE because that is the vocabulary a reader already has from Kafka, SQS and Redis Streams, and because KEY is what `hash_to_partition()` routes on, which STREAM does not describe. Renaming those two and moving TIMESTAMP is the *whole* divergence budget for the field layout: see [ADR-2](architecture-decisions.md#adr-2-one-message-format-the-7-field-positional-array). Anything further has to be justified separately. The eighth slot differs in kind rather than in spelling: a provenance taint where upstream keeps a thaw flag, and one that never crosses a process boundary, which is what makes its presence worth trusting ([ADR-15](architecture-decisions.md#adr-15-command-authorization-local-taint--the-minter-signs)).
...
`packed()` and `unpacked()` are a JSON encode and decode of the same positional array, where Tachikoma packs binary and carries `TM_STORABLE` and a freeze/thaw state (`IS_UNTHAWED`) for Perl `Storable` payloads.

**Why:** the wire shape has to be the memory shape in *two* languages. A Storable equivalent has no JavaScript peer, so it would reintroduce exactly the per-boundary translation layer ADR-2 exists to remove.

### No TM_PERSIST, no `answer()` / `cancel()`, no `max_unanswered`

![The five nodes ADR-3 reaches, side by side, and MemorySieve upstream alone. Tachikoma: a producer sends TYPE | TM_PERSIST through an output_buffer that can fill to a consumer, which replies TM_PERSIST | TM_RESPONSE with PAYLOAD 'answer' or 'cancel', and the producer stops at max_unanswered; Null.pm doubles as a load generator, Tee.pm keeps a ledger expired by a timeout, Grep, AgeSieve and PayloadTimeout cancel what they refuse, and MemorySieve.pm cancels at the pipeline entry once a buffer reaches max_size. Newspack Nodes: a producer's fill() returns void into a Partition on disk, a Consumer owns its cursor, and nothing comes back; Null_Node only counts, Tee_Node keeps no ledger, the filters return without forwarding, and MemorySieve has no counterpart. Beneath, the browser's dumper-node.js ring, the one place the shape returns.](img/tl-no-persist.png)
...
`TopicProbeToGraphite.pm` parses TM_BYTESTREAM `key:value` lines, tracks partitions and consumers in two passes so it can derive `distance` from the difference between them, keeps the latest topic hash per consumer, and emits two fields — `distance` and `msg_sent`, the cumulative node counter. [`Probe_To_Graphite_Node`](../includes/class-probe-to-graphite-node.php) reads a positional `Probe_Record` off a TM_STRUCT message with DISTANCE already computed, so it needs no second pass, and it emits four fields: `distance`, `msgs_delta`, `bytes_read_delta` and `cache_size`. Two of those four accumulate rather than replace.
...
[`Age_Sieve_Node`](../includes/class-age-sieve-node.php) reads a `max_age` of zero or below as "not supplied" and falls back to 900 seconds. Upstream's `AgeSieve.pm` keeps the literal zero, through the `//` inside its `arguments()` setter.
...
![Two lifecycles side by side. Tachikoma is one process started by bin/tachikoma-server, holding _router and Select.pm's reader, writer and watcher descriptors, with anything that can block forked into a Job over a socketpair on FD 5. Newspack Nodes is a request: POST /newspack-nodes/v1/workers/spawn, fired and forgotten on a 250 ms budget, runs the graph inline under ignore_user_abort( true ) and set_time_limit( 0 ); it claims locks/<type>.p<N>/ by mkdir and a heartbeat file, stale after 60 seconds by default, and in a finally releases the lock and POSTs its successor, at most 595 seconds later. A killed worker gets no finally; a peer's 15-second _fleet scan or the minute cron pass spawns the successor. Beneath, three kinds of new work with no Perl: WordPress as the sink, request-owned boundaries, and compositions of our own.](img/tl-no-upstream.png)

[ADR-8](architecture-decisions.md#adr-8-worker-zombie-pattern) and [ADR-9](architecture-decisions.md#adr-9-two-tier-safety-net) record why there is no supervisor and no reaper, and [ADR-12](architecture-decisions.md#adr-12-dead-letter-poison--crash-lifecycle) why crash isolation comes from the dead-letter lifecycle rather than from a forked Job. Everything that talks to WordPress is new work too, and so are our own compositions: each carries Tachikoma's contract and none of its code, from [`Hook_Node`](../includes/class-hook-node.php) to the request-scoped boundary nodes and the async-job path, all listed in the diagram. One needs more than the diagram holds:
...
|---|---|
| `InstrumentalityGrail.pm` | event-logger-nodes' [`Request_Builder_Node`](https://github.com/Automattic/newspack-event-logger-nodes/blob/v0.95.3/includes/class-request-builder-node.php). Assembles a request's log lines into one record and validates the per-request `n` sequence, so a mid-stream orphan, a duplicate, a reordering, or a reused request id is caught rather than quietly assembled into a plausible-looking record. |
| `InstrumentalityFlight.pm` | event-logger-nodes' [`Request_Flight_Node`](https://github.com/Automattic/newspack-event-logger-nodes/blob/v0.95.3/includes/class-request-flight-node.php). A hidden timer sibling that snapshots its patron's in-progress request map on each tick and emits the live in-flight set. |
| `ReqGrep.pm` | [`LRU_Cache`](../includes/class-lru-cache.php), and event-logger-nodes' [`Reqgrep_Core`](https://github.com/Automattic/newspack-event-logger-nodes/blob/v0.95.3/includes/class-reqgrep-core.php) behind `wp nodes reqgrep`. A `Table.pm` subclass that accumulates a request's lines under one key through `lru_lookup`, which is the shape `LRU_Cache` takes and the reason a hit promotes its entry. |
...
- [`AGENTS.md`](../AGENTS.md) — the contract summary, including the standing rule: match Tachikoma's model, don't blind-copy its field names.
