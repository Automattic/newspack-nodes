...
Control rides the same rails as data. A command is a message carrying the TM_COMMAND bit, its value a verb name and a flat list of string arguments, and it routes by TO. Every node sinks into a `Command_Interpreter_Node`, the process's `_command_interpreter`, which holds a table of verbs and, once TO is peeled empty, looks the verb up and calls its handler. A service like the topology catalog subclasses `Service_CI_Node`, declaring each verb with a handler and a role.
