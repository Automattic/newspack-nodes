...
**Shared `Core::$memd` and cache policy.** The one process-wide `\Memcached` handle is built by the substrate itself (`Bootstrap::init_memcached`, run at plugin-file scope, since 0.12.0) from the substrate's own `memcache_servers` config — not by an application plugin. Empty/invalid config leaves `Core::$memd` null rather than installing an unreachable fake connection. `Cache_Backend` then makes the policy explicit per caller: `local_first()` selects APCu then Memcached, `shared_first()` selects Memcached then usable APCu, and `shared_only()` accepts Memcached only. SSE slots, tables, batch counters, and stats use `shared_first()`, so they can remain functional inside one web host through APCu when Memcached is absent; that fallback is not visible to another host or to a CLI/worker process with a separate or disabled APCu segment. Command sessions use `shared_only()` and therefore still refuse and log without Memcached. If neither selected backend exists, each caller keeps its documented fail-open, fail-closed, or fail-soft behavior.
