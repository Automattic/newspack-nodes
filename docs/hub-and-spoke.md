...
- [`newspack-event-logger-nodes/topologies/aggregator.tsl`](https://github.com/Automattic/newspack-event-logger-nodes/blob/v0.96.0/topologies/aggregator.tsl), a hand-written `Remote_Source` leg per spoke — the form a `Vault_Group` node builds from the Vault's `group` field instead
