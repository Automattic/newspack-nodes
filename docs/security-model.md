...
The scope is a random per-install salt rather than a derivation from the site secret, which is what `session_address()` does for command sessions; its comment says *"the cache is shared infrastructure, not a trusted store."* Two facts argue against copying it. Folding `wp_salt('nonce')` into `site()` risks the split keyspace `site()`'s docblock warns about, and `wp_salt()` is unavailable under the SHORTINIT boot, which stops before `pluggable.php`; `get_option()`, which `salt()` reads, is defined ahead of that bail. Rotating that salt is the logger's schema migration; the logger's [security model](https://github.com/Automattic/newspack-event-logger-nodes/blob/main/docs/security-model.md#tradeoffs) records the choice.
