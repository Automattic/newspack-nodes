...
Do [writing-a-plugin.md](writing-a-plugin.md) first if you haven't — this guide assumes the digest pipeline, in which sources feed a summarizer that feeds the digest, and the `fill`/`sink`/`target`/`node_schema` vocabulary.

> **Diffing against the shipped code — the `_Demo` suffix.** The teaching snippets use bare names (`Scorer_Node`, `Insights_CI_Node`, …), but the bundled example carries a `_Demo` suffix on every class — `Scorer_Demo_Node`, `Insights_CI_Demo_Node`, files `class-*-demo-node.php`, namespace `Example_AI_Newsletter` — to deconflict from the real sibling plugin (`newspack-intelligence`) that can be loaded in the same WP. Likewise the topology file is `topologies/example-ai-newsletter.tsl` (name `example-ai-newsletter`), the durable log is `example-scored.p*`, and the mounted service CI node is `insights-demo`. So when you diff against [`examples/example-ai-newsletter/`](../examples/example-ai-newsletter/), map each bare name to its `_Demo` form. The JS carries no suffix and needs none, but it does not line up name for name with the real plugin either: both ship a `src/dashboard/PublisherInsightsPage.js`, while newspack-intelligence's hook is `useInsightsGraph` and its third widget is `AccumulatedPanel`, which owns the newsletter actions — Collect, Regenerate digest, Copy markdown and Create draft post — leaving its `TopTable` the ranked table and no buttons at all. Here the hook is `usePublisherInsightsGraph`, the third widget is the passive `AccumulatedCard`, and §5's actions sit on `TopTable`. Read the widget names before assuming a file is the one you want.

> **Every section below ends at a primitive in the substrate** — `enqueue_react_page`, `buildDashboards`, `createJestConfig`, `Fetcher`, `read_latest_snapshot_cache`, `useBatchedPoll`, `addSliceFetcher`. Each owns 20–250 lines the example would otherwise copy, and each sits in the substrate by the rule the first guide follows: **when a step feels like boilerplate, the fix belongs in the substrate, not the tutorial.** Where a step is one call, this guide says what the call spares you, so you can see the seam.
...
3. **Render it** — the real JS node graph, a Timer feeding a Tee that fans out to three Fetchers, each answering into its own view node, three thin widgets, the build, the enqueue.
...
> Each PHP file below opens with the same preamble [writing-a-plugin.md §2](writing-a-plugin.md#2-the-first-node--a-source-anas-story) established — `namespace Newspack_AI_Newsletter;`, then one `use Newspack_Nodes\<Class>;` line per class it needs. §1's nodes need `Node`, `Message` and `Core`; §2's service interpreter needs `Core`, `Service_CI_Node`, `Partition_Node` and `Config`. The snippets show only the class body.
...
class Scorer_Node extends Node {
	...
	/** Palette tile and argument form. The weights are a constant, so there's nothing to configure. */
...
It slots between the summarizer and the digest, so the summarizer now feeds the scorer. Nothing else changes — the summarizer never learns there's a score, the digest never learns how it was computed. Same lesson as Ben's source in [writing-a-plugin.md §6](writing-a-plugin.md#6-the-reveal--a-second-source-bens-story). The toy scorer carries no clock and no randomness, so its suite asserts exact scores rather than ranges; an LLM call belongs in `score()` and nothing else in the file changes when it arrives.

A pure transform still declares its schema, and those six keys are the whole of it — a category, a description, and the two flags saying it takes a `fill` and honours a `target`. Omit `node_schema()` and the class inherits `Node`'s default, whose `category` is the empty string, and the console's class catalog refuses an empty category exactly as it refuses `Hidden`: no palette tile, no inspector entry. `make_node Scorer` still builds the node, so the omission costs you the console and shows no other symptom.

The two guards at the top of `fill()` are a **whitelist**, not a filter: a message that is neither TM_STRUCT nor an array VALUE dies here rather than passing through. This example mints nothing else, so nothing breaks. Add a control signal — a source's terminal `DONE`, which [writing-a-real-plugin.md](writing-a-real-plugin.md) uses to auto-compose the digest — and the scorer strands it. The real `Scorer_Node` in newspack-intelligence therefore opens `fill()` with one more branch, `if ( $type & Message::TM_INFO ) { parent::fill( $message ); return; }`, forwarding the signal untouched before the scoring work. That is the line you add the day your pipeline gains control traffic.
...
![The durable-snapshot mechanism: in the worker, scorer appends to scored:partition's log, scored:consumer tails it into digest, and each checkpoint writes one offsetlog frame carrying the read cursor, the poison accounting, the consumer's identity and a cache key holding digest's save_state(); a web request's items() reads the newest frame through Partition_Node::read_latest_snapshot_cache. Below, the four conditions that decide whether a frame is written: the 30-second checkpoint interval with its cursor-moved guard, a FLUSH that moves no cursor and so writes no frame, the clean-shutdown checkpoint that skips the guard, and the one-interval freshness lag; then the lockstep restore on respawn, the warning a name that cannot restore logs, and the three facts that keep the snapshot bounded.](img/wd-durable-snapshot.png)
...
The sheet's lower half is the cadence to read before trusting a card: [`Durable_Reader::fire()`](../includes/trait-durable-reader.php) checkpoints only once `Dead_Letter_Queue::CHECKPOINT_INTERVAL_S` has elapsed **and** the cursor moved, so the page renders what the last commit held, which is not the same as what the worker holds now.
...
The browser can't read a file off the worker's disk. It speaks the **command protocol**: a `POST` of a `TM_COMMAND` to [`/newspack-nodes/v1/command`](API.md#command-dispatch), which the substrate routes to a node by name. So we mount a **service interpreter** into every web request and give it verbs.

The decomposition decision here is the same one we make on the client: a single `insights` verb returning `{sources, top, accumulated}` would be a **god command** — one verb that computes everything, that no one can reuse a slice of, that the dashboard can only fetch all-or-nothing. So `Insights_CI` exposes **three small verbs**, one slice each:
...
**All three count the same list, and that list is the digest's *unflushed* buffer.** It is the `items` the digest's `save_state()` handed the Consumer at its last commit, so `accumulated` is a pending count, not a lifetime total, and `counts` and `top` describe the same pending items. `request_node digest FLUSH` — the runtime trigger [writing-a-plugin.md §4](writing-a-plugin.md#4-the-digest-builder--accumulate-then-flush-and-reuse-log) teaches you to type at the REPL — empties `$items` after emitting the draft, so once the Consumer's next cursor-advancing checkpoint commits the emptied list, **By source**, the score-ranked table and the **Total items** card all drop to zero together and climb again as new scored records arrive. The dashboard's own **Draft newsletter** button does *not* flush: it renders the markdown in the browser from the `top` slice it already holds, and leaves the numbers alone.

A service interpreter is a [`Service_CI_Node`](../includes/class-service-ci-node.php) — a `Command_Interpreter_Node` whose verb table comes from its `node_schema()`, the same double-duty schema you already write for nodes. `includes/class-insights-ci-node.php`:
...
> **← two Partition readers.** [`Partition_Node::read_latest_value_at( $offsetlog_dir )`](../includes/class-partition-node.php) reads the newest committed record's VALUE from an offsetlog in one call. It wraps the 20-line segment walk you would otherwise write — construct a `Partition_Node`, set `arguments`, `get_segments( true )`, take the newest segment, `read_at`, split lines, `Message::unpacked`, pull `VALUE` — and falls back a segment when the newest one rotated but was never written. **`Partition_Node::read_latest_snapshot_cache( $offsets_dir, $glob, $node )`** does the rest of the descent on top of it: it globs the `p*` snapshot dirs, descends into each `cache[<node>]['items']`, and flattens the lists (`$cache_key` and `$items_key` default to `cache` and `items`). That is why `read_snapshot_items()` above is one line: you ask the Partition for its latest snapshot instead of walking segments. It re-globs on every call, which is why `items()` memoizes.
...
A read-only dashboard can widen that gate by declaring the role on the verb — `'capability' => \Newspack_Nodes\Capabilities::READ` beside `'name'`, which is what the substrate's own dashboard verbs do. The three roles are `read` (dashboards, SSE, introspection), `tune` (declared configuration and application data) and `manage` (fleet control and credentials); all three resolve to `manage_options` until a site filters one down through [`newspack_nodes/capability_map`](API.md#filters) or an operator runs [`wp nodes caps install`](cli.md#capabilities-and-the-hub-user).
...
The `require_once` is not the autoloader repeated. Composer's classmap is refreshed only by a hand-run `composer dump-autoload -o`, so a class added or renamed since the last dump would have `make_node` answering `unknown class` from inside a live request. Requiring the file makes the mount independent of how fresh the classmap is — which is why §8's `composer dump-autoload -o` step notes this one node as its exception. Delete the line and you have coupled every dashboard request to a build step nobody runs.
...
## 3. The JS graph: a Timer, a Tee and three Fetchers, on the same `fill()` contract

Here is the part that surprises people: **the browser runs the same node runtime.** [`@newspack-nodes/runtime`](../src/runtime/index.js) is the JS port — `Node`, the `Message` field constants, a `CommandInterpreterNode`, [`mountExospine`](../src/runtime/exospine.js) (which clips your nodes onto the standard `_command_interpreter → _router` backbone). So the client graph is *real*: a `Timer`, a `Tee`, three `Fetcher`s, three receiver `Tee`s, three view nodes. Message traffic at every edge.

### a. The `Fetcher` — the composition primitive

The piece that makes fan-out work is the generic **[`Fetcher`](../src/runtime/fetcher-node.js)** — a runtime node registered under the type name `Fetcher`, so you reach it through `makeNode`, not an import:
...
Each receiver `Tee` fans the reply **back to its own Fetcher** as well, which is what takes the ask off that Fetcher's outbox. [`addSliceFetcher`](../src/shared/helpers/addSliceFetcher.js) connects the Fetcher **last**, after the view, because a Tee fans out in connect order and the ask must still stand while the reply renders. Until the ask settles, the Fetcher is quiet: a one-second refresh on a four-second verb asks once and waits, instead of stacking four identical commands the server is still working through. An unanswered ask is re-sent after `retry_after_s` (0 disables the re-ask, which is what a write wants) and retired at 120 seconds, so a lost reply costs one slow refresh rather than a dead widget; §4's sheet draws both timers.

That receiver belongs to its Fetcher alone. Mint a send straight from a receiver — outside the Fetcher polling through it — and that reply settles whatever ask the Fetcher had outstanding, because a pathless ask matches any pathless reply. The cost is one extra command in flight, and the next tick levels it out; where one ask at a time has to be exact, give the out-of-band send a receiver of its own.
...
> The `errorMessage` import is a substrate helper too: **[`@newspack-nodes/shared/errorMessage`](../src/shared/errorMessage.js)** coerces a `TM_ERROR` payload (string / `{message}` / anything) to readable text in one place every dashboard view node shares.
...
export const views = registerSliceViews( {
	SourceCountsView: { json: true, empty: { sources: {} } },
	TopTableView:     { json: true, empty: { top: [] } },
	AccumulatedView:  { json: true, empty: { accumulated: 0 } },
} );
```

`json: true` is what these three verbs require, and it is the one flag a subclass never needs: `slice_verb()` JSON-encodes, so the payload lands as a string, and a declaration decodes it only when told to — the substrate's own Aggregator views set the flag for the same reason, while the Vault and Sessions catalogs answer a live struct and omit it. A declaration also takes a `parse` that reshapes the decoded body, and a `description` for the palette tile.

A substrate view that is only its slice is declared this way, the Topology Console's two catalogs among them. Subclass only for a view that owns more than its slice: its own `fill()`, a ring buffer, a timer, a teardown. That is what the Event Dashboards bundle writes out beside its one declaration, and the example keeps one file per view because a tutorial has to show the class before it shows the shorthand. Either way the classes are what the graph is built from, and the names are for TSL and the console palette.
...
> **Hand `makeNode` the CLASS, not the name** ([ADR-16](architecture-decisions.md#adr-16-js-node-class-resolution--names-are-the-tsl-surface-classes-are-the-api)). `includeNodes` is a per-bundle static, so a name resolves only through an interpreter *this* bundle mounted; a station tab building its graph through another bundle's interpreter would find nothing. The registration above still earns its keep, for the two paths that can only spell a name: a `make_node SourceCountsView` typed at the REPL or replayed from TSL text, and `help SourceCountsView`, which reaches the schema through the same `includeNodes` lookup. The console palette is not one of them — `SliceViewNode.nodeSchema()` declares `category: 'Hidden'`, every subclass and every `sliceView()` declaration inherits it, and the class catalog drops a Hidden category before it builds a tile. §4's `SLICES` table carries the imported classes.

---

## 4. Mount the graph and poll it — the batched-poll toolkit

The hook builds the real graph and owns the poll loop. The shortcut it refuses is "mount one view node and fire one `poll` command" — that *is* the god pattern, the convenience behind every god-object dashboard. We're composing a graph, so we reach for the substrate's batched-poll toolkit instead: **`useBatchedPoll`** owns all the mount boilerplate, and **`addSliceFetcher`** wires one slice in one call. The hook is then its slices and nothing else.
...
Four options ride in the same bag. **`intervalMs`** is required and at least 1000 ms, for the reason the sheet's red box gives; changing it re-arms the Timer live, which is how the Aggregator dashboard wires it straight to a refresh dropdown (`intervalMs: parseInt( refreshInterval, 10 ) || parseInt( DEFAULT_REFRESH_MS, 10 )`). **`paused`** suspends polling without unmounting the graph; the Overview dashboard passes `paused: dragging` so a poll doesn't fight a drag in flight. **`enabled`** gates a tab nobody has opened. **`passenger`** clips onto a backbone somebody else owns, for a poll that is *part* of a page rather than its graph: the owner keeps the full rebuild, and a passenger re-attaches when the backbone comes back. Sub-second work belongs to [`useRouterTick`](../src/shared/hooks/useRouterTick.js).

A slice can also emit *live* command args per tick: `addSliceFetcher` takes an `argsFn: () => argsTokens` fire-time getter, returning the flat token array `formatCommandArgs` builds, that it assigns to the Fetcher's `command_args`. A `null` return sends nothing that tick. The toy's three verbs take no args, so it needs none.
...
`TopTable` also owns the newsletter actions — the **Draft newsletter**, **Copy markdown** and **Create draft post** buttons — because they operate on *its* `top` items. Both documents are composed in the browser; only **Create draft post** leaves it, and it goes to the WP REST API (`POST /wp/v2/posts`), never back through the node graph. That call sits behind a `createDraft` prop defaulting to `apiFetch`, so a test hands the component a fake that resolves or rejects and exercises both rendered outcomes without touching the network.

Read which button drives which builder, because the function name and the first label do not pair up. **Draft newsletter** sets local state and renders the on-screen `<ul>` preview from the `top` array; **Copy markdown** is what calls `draftNewsletter()` and hands the result to `navigator.clipboard.writeText`; **Create draft post** calls `newsletterPost()`.

**An action's result lives in React state, never in the slice.** The preview array, the "Copied" flag, the "Edit draft" link, the draft error and the in-flight flag are all `useState` on the widget, because §3d's `SliceViewNode` rebuilds its model whole on every reply — `{ ...settled, ...slice }` — so anything published into the slice, including through `controlMsg()`, is gone at the next poll thirty seconds later. The rule that follows: a field belongs in the slice only when the next reply *should* retire it, as it should retire a spinner or a refusal, and anything the user has to go on seeing across polls belongs in component state. The preview holds the array captured at the click for the same reason, so a later poll refreshes the table beneath an unchanged preview.

Two of the three buttons can fail, and each degrades rather than dying quietly. **Copy markdown** tests `window.navigator.clipboard` and its `writeText` before calling either — the API is absent on an insecure origin and on older browsers — and renders "Clipboard unavailable here — copy from the preview instead." where an unguarded call would leave a dead button and nothing in the console; it flags "Copied" only once the `writeText` promise resolves, since a denied permission or an unfocused document still rejects it. **Create draft post** stays `disabled` across the round trip, so a second click cannot open a second draft, and it has three outcomes rather than two: a reply carrying an `id` builds the link, a reply without one gets an error notice instead of a `post=undefined` link, and a rejection surfaces its own `message`. The link itself is `${ window.location.origin }/wp-admin/post.php?post=<id>&action=edit`, which assumes the site sits at the domain root under the default admin path — `origin` drops a subdirectory install's path, and the link 404s there.

Those three actions share one tiny resolver so the preview, the markdown draft and the draft-post HTML agree on what an empty field reads as, `src/dashboard/itemLabel.js`:
...
Three things about those four lines. It **resolves**, it does not coerce: a truthy field passes through as it stands, so an item whose `title` arrives as a number hands the caller a number, and the text is raw — each renderer escapes or formats for its own target. `||` rather than `??` is deliberate on `title`, because §2's `shape_top()` already writes `'title' => $item['title'] ?? ''` and leaves the replacement to the client, which needs `__()`; `??` would let that empty string through and blank the preview, the markdown and the draft post at once. The `'?'` on `source`, in contrast, repeats a literal `shape_top()` also writes, so the same placeholder lives in two languages and the two must stay in step — this arm only catches the empty string or non-string the PHP `??` misses.

The three actions are the only consumers. `TopTable`'s own table cells print `item.source` and `item.title` straight off the slice, so an item that `shape_top()` gave an empty title renders a blank cell in the table and `(untitled)` in the preview two inches below it.
...
Its sibling `src/dashboard/newsletterPost.js` builds the draft post's `{ title, content }` from the same items, as an HTML list. It escapes the five HTML-significant characters itself rather than leaving that to WordPress: an administrator on a single site holds `unfiltered_html` and is exempt from kses, so an item title carrying markup would be stored as markup. Its `escapeHtml()` opens with `String( value )`, and that coercion is load-bearing rather than habit — `itemLabel` hands back the item's own field whenever it is truthy, and a numeric title has no `.replace()`. `draftNewsletter` escapes nothing, because its destination is the clipboard.

`content` is classic HTML — a plain `<ul>` of `<li><strong>title</strong> — source</li>`, with no block delimiters — so the created draft opens in the editor as one Classic block. That is the cheap shape, and it is a choice: newspack-intelligence's `markdownToBlockMarkup()` runs its text through `pasteHandler` and `serialize` to store real core blocks instead. Pick the second when the draft is meant to be edited as blocks.

`PublisherInsightsPage` reads `import './styles/insights.scss'` — create that file. Style it to the **Newspack in-product design system**: Cobalt (`#003DA5`) for the primary action, neutral surfaces (`#fff` / `#f7f7f7`) and borders (`#ddd`), `#1e1e1e` / `#6c6c6c` text, Inter, the 4/8/16/24/32px spacing scale, and functional colors (error `#B32D2E` on subtle `#FCF0F1`) only for status. The example's `insights.scss` is the reference. **Lay it out in flow** — a normal block in the admin content column — *not* `position: fixed` / full-bleed: that overlay pattern belongs to the Topology Console and the station (which deliberately take over the viewport), and on a standalone admin page it hides the WP admin bar and menu.
...
// Hand-matched to §7's INSIGHTS_MOUNT_ID. No constant crosses the PHP/JS
// boundary, so changing one side alone leaves the page blank with nothing logged.
...
The listener still fires because `Admin::enqueue_react_page()` enqueues the bundle **in the footer**, below the mount div: a footer script runs while the document is still parsing, so `DOMContentLoaded` has yet to fire. The `if ( el )` guard is not there because the div might be missing — the registrar's `?page=` gate means the bundle loads only where the div is printed — but because `createRoot( null )` throws. A bundle injected *after* load misses the event entirely and must mount at module evaluation instead, which is what [`src/station/index.js`](../src/station/index.js) and event-logger-nodes' [`src/requests/index.js`](https://github.com/Automattic/newspack-event-logger-nodes/blob/v0.96.0/src/requests/index.js) do: they arrive through the lazy tab path, `Admin::lazy_tab_script()` and the `NewspackNodesLazyTabs` map. Converting this page into a `host: 'station'` tab ([writing-a-real-dashboard.md](writing-a-real-dashboard.md)) means making that swap.

That is the entire view layer: three thin widgets reading three nodes, two client-side document builders over one shared resolver, and one mount.
...
**That sibling has to be a git clone.** The release zip carries the runtime alone — `includes/`, `topologies/`, `build/` and a production `vendor/` — because [`.distignore`](../.distignore) drops `src/`, `scripts/`, `tests/` and `docs/`. So `wp plugin install newspack-nodes.zip`, the install [getting-started.md](getting-started.md) walks you through, leaves a substrate at `wp-content/plugins/newspack-nodes/` with no `src/` in it — at exactly the path the default resolves to. The build then stops on `substrate src not found at …`, and pointing `NEWSPACK_NODES_SRC` at that same directory changes nothing. Clone the substrate repository beside your plugin, or point `NEWSPACK_NODES_SRC` at that clone's `src`. You need the clone three more times: the `jest.config.js` below spells it literally in both the `require()` and `aliasBase`, since jest reads no environment variable; [writing-a-plugin.md §8b](writing-a-plugin.md#b-test-it--the-bootstrap-is-the-only-non-obvious-part)'s test bootstrap loads the substrate's `tests/Helpers/`; and §8c's PHPStan config scans `../newspack-nodes/includes`.

The guard moves from the kit file onto the directory, because the directory is what the variable names: a wrong `NEWSPACK_NODES_SRC` should say so rather than report a missing `index.mjs`. The `buildDashboards` call below it changes in one place — `nodePaths` becomes your own `node_modules` (`path.resolve( ROOT, 'node_modules' )`), since the substrate checkout beside you may have none. A plugin that declares dependencies of its own, as both standalone consumers declare `d3` and `@noble/hashes`, needs one more loop to pin them; [writing-a-real-dashboard.md §7](writing-a-real-dashboard.md#7-build--deploy-realities) carries that loop and the 88KB duplicate it prevents.

![The build resolution in four stages: the in-repo and standalone build.mjs each name the substrate's src, one at a fixed depth and one through NEWSPACK_NODES_SRC with the four retired overrides refused; alias-map.cjs maps the three @newspack-nodes aliases from that base for esbuild and jest alike; buildDashboards rewrites the ten WP_EXTERNALS to window globals, bundles everything else, and writes index.js, index.asset.php, index.css and index-rtl.css; and Admin::enqueue_react_page gates on the page slug and file_exists, reads the manifest, enqueues in the footer, versions the stylesheet by its own hash and localizes NewspackNodesData. A side card stacks the theme, ui and graph stylesheet handles.](img/wd-build-resolution.png)

The silent null in the sheet's fourth stage is worth recognising, because a checkout or a release zip that never ran `npm run build` produces the identical symptom to a renamed entry. The menu callback still prints its empty mount div inside `.wrap`. The debug overlay is no help either — §9 mounts it from inside `PublisherInsightsPage`, so it lives in the very bundle that failed to load. Confirm `build/dashboard/index.js` exists on the server before looking anywhere else.

[`src/build-kit/alias-map.cjs`](../src/build-kit/alias-map.cjs) is the single place the three aliases are written down, and [`src/build-kit/index.mjs`](../src/build-kit/index.mjs) holds `buildDashboards` and the `WP_EXTERNALS` map beside it. [browser-runtime.md](browser-runtime.md#one-build-kit) covers the four consumers that come off the kit and why a green release run proves nothing about which substrate shipped.
...
`pinReactFrom` names the one `node_modules` all four React specifiers resolve through — `react`, `react-dom`, `react/jsx-runtime` and `@wordpress/element`. A standalone plugin carries its own copy of those, beside the substrate sources jest maps through the `@newspack-nodes/*` aliases, and every substrate hook you mount reaches `useState` and `useEffect` through `@wordpress/element` — `DebugOverlay` among them. Let both copies load and those hooks run against a different React instance than your own components use, which fails on the first render with `Invalid hook call`. The substrate's own `jest.config.js` omits the option because there is one `node_modules` in play and nothing to disambiguate.
...
Ahead of your `jest.setup.js` the factory loads one of its own, [`src/build-kit/jest-node-timers.js`](../src/build-kit/jest-node-timers.js). It holds every runtime node your suite mounts to two timer invariants, and breaking either fails the test:
...
The example's `jest.setup.js` earns three more blocks, and a dashboard suite wants all three. It **authenticates the command session** in a `beforeEach` (`forgetSession`, `__setAuthFetch`, `ensureSession` from `src/runtime/command-auth`) — every Fetcher holds until `readyToMint()` says yes, which is what production does, so without it every poll test asserts silence. It installs Node's `TextEncoder` and `webcrypto`: jsdom supplies no `TextEncoder` at all, which the signer reaches through `@noble/hashes`' `utf8ToBytes`, and jsdom's `crypto` — carrying `getRandomValues`, which the signer's nonce needs, but no `subtle` — gives way to Node's whole object, the missing `subtle` being the tell that it is jsdom's. The signer itself never calls `subtle`; its HMAC comes from `@noble/hashes` because it must be synchronous. And it **fails any test emitting an unexpected `console.warn` or `console.error`**, letting through only the substrate's own timestamped `Core.stderr()` lines.

That third block is the **loose** variant of the gate, and a real plugin should carry the strict one. The example discards every warn matching the `SUBSTRATE_STDERR` prefix unread, so a node fault, a `printLessOften()` line or a dropped-message notice vanishes and a dashboard test whose graph genuinely faults passes green with no trace. The substrate's own `jest.setup.js`, and event-logger-nodes', close that hole with a declaration: `global.expectConsoleWarn( 'Router: dropped message not addressed' )` suppresses exactly the warn a test means to provoke, an undeclared substrate warn fails the test, and a declared one that never fires fails it too. Copy that setup, not this one, the moment your suite exercises a fault path.

`SUBSTRATE_STDERR` is itself a hand-maintained copy — the same regex sits in six `jest.setup.js` files across these plugins — of the prefix `localStamp()` in [`src/runtime/core.js`](../src/runtime/core.js) builds from `Intl.DateTimeFormat( 'en-CA', { timeZoneName: 'short' } )`. Its zone alternation (`UTC`, `GMT±H[:MM]`, or a two-to-five-letter abbreviation) tracks exactly what that emits. Change the stamp and the exemption breaks in every copy at once, surfacing as unrelated suites failing on substrate spam. Widening the zone token to a bare `\S+` does the opposite and makes the gate swallow the ordinary `<date> <time> <word> <word>: ` warnings it exists to report.

Tests drive the graph through the **wire**, not the client: `installFakeCommandWire( replyFor )` from [`@newspack-nodes/shared/test-utils/fakeCommandWire`](../src/shared/test-utils/fakeCommandWire.js) stubs `global.fetch` (and `/auth`), so packing, `HttpOut`, the Router and the interpreter all run for real and replies come back `TO = FROM`. `wire.batches` is what was posted, which is how the example asserts that one tick is one POST carrying three commands. Its companion `runClockFast( factor )` from [`test-utils/fastClock`](../src/shared/test-utils/fastClock.js) shadows `Core.now()` so a test reaches the next poll boundary instead of waiting out a 30-second cadence.
...
```
(Plus the dependencies any React/esbuild project needs — `esbuild`, `sass`, `rtlcss`, `jest`, `jest-environment-jsdom`, `babel-jest`, `@testing-library/*`, `@wordpress/element`/`i18n`/`api-fetch`, `react`/`react-dom`; copy the example's `package.json` and `babel.config.js` rather than hand-rolling them.)

`babel.config.js` is read by jest and by eslint's parser; esbuild reads none of it. Babel resolves a root config from the **working directory** rather than from the file being processed, so whichever Babel-driven tool you launch from the plugin root reads yours: `babel-jest` when the suite runs, and `@babel/eslint-parser` when `eslint .` or an editor lints from there. (`@wordpress/eslint-plugin`'s `esnext` config injects its own preset only when it finds no babel config; with one present, the parser reads yours.) That makes both settings load-bearing beyond the test run:

- **`@babel/preset-react` with `runtime: 'automatic'`** matches the kit's esbuild `jsx: 'automatic'`, so JSX resolves through `react/jsx-runtime`. Nothing under `src/` binds `React` — the dashboard imports `createRoot` and `useState` from `@wordpress/element` — so the classic runtime would fail every JSX file on an undefined `React`, in lint as well as in test. This is why the `.eslintrc.js` below sets no `parserOptions`.
- **`targets: { node: 'current' }`** compiles for the Node running the suite. Without it `@babel/preset-env` falls back to browserslist resolution, which walks *up* the tree: this package declares no `browserslist` key, so it would inherit the substrate's two levels up and down-compile the tests to the WordPress browser matrix for nothing. An explicit `targets` overrides an own key and an inherited one alike.

The eslint config splits the same two ways `build.mjs` and `jest.config.js` do. **In this repo**, the substrate's `.eslintrc.js` is `root: true`, so an example inherits the whole ruleset — including the `import/core-modules` and `import/no-unresolved` settings that carry `@newspack-nodes/runtime`, `@newspack-nodes/shared/*` and `@newspack-nodes/debug-overlay` past the resolver — and `examples/example-ai-newsletter/.eslintrc.js` restates one rule and adds one ignore:

```js
module.exports = {
	rules: {
		'@wordpress/i18n-text-domain': [ 'error', { allowedTextDomain: [ 'example-ai-newsletter' ] } ],
	},
	ignorePatterns: [ 'build/' ],
};
```

The text domain is the only thing it *must* restate: the root config allows `newspack-nodes` alone, while §5's strings read `__( 'Publisher Insights', 'example-ai-newsletter' )`. Restating a rule with options **replaces** the inherited options rather than merging them, which is the intent here — no file in the example may use the substrate's domain. Without those two keys `npm run lint:js` errors on every `__()` in the example, and `pre-push` runs `lint:js` on any JS push, so the whole repo is blocked. The `build/` ignore is not for `lint:js`, whose globs reach only `examples/*/src` and `examples/*/scripts`; it covers `eslint .` from the plugin root and an editor linting the open bundle. **A standalone plugin** inherits none of that and needs a full `root: true` config carrying the `@newspack-nodes/*` resolver exemptions itself — `newspack-intelligence/.eslintrc.js` is the shape to copy.

> **← what the two factories own.** **`buildDashboards()`** owns the ~250 lines a consumer's `scripts/build.mjs` would otherwise carry — the `@wordpress/*`→global externals plugin, the SCSS plugin, the asset-manifest emitter, the RTL pass, the watch/one-shot orchestration — and the substrate, the example, event-logger-nodes and intelligence each call it. esbuild/sass/rtlcss are *injected*, so the kit builds against a substrate checkout with no `node_modules` of its own. The jest config carries a real footgun — the `@newspack-nodes/shared` mapper is listed **before** the `\.(css|scss)$` style-mock, and first match wins, so a style imported *through the alias* (`@newspack-nodes/shared/styles/x.scss`) resolves to the real file and babel-jest crashes parsing SCSS as JS instead of mocking it. **`createJestConfig()`** bakes that order in, and shared components import their own styles by relative path so the mock catches them. `npm run build`, `npm run test:js`, done.
...
```php
// The `?page=` slug WordPress routes on, and the element id §5's index.js looks
// up. They hold the same string and answer different consumers, so each is its
// own constant: changing one implies nothing about the other.
...
function register_insights_admin_page(): void {
	...
	\add_menu_page(
		...
		static fn () => print( '<div class="wrap"><div id="' . \esc_attr( INSIGHTS_MOUNT_ID ) . '"></div></div>' ),
...
`version_fallback` is the script's version in the two cases §6's sheet names where the manifest cannot answer. Pass your **own** plugin version, as the example does: the registrar's own fallback is `NEWSPACK_NODES_VERSION`, the substrate's version rather than yours, so a rebuild against an unchanged substrate would serve a stale bundle from cache.

`page` takes one slug or a list of them, which is how a single bundle covers several admin pages; the station's tab-bundle path passes a list for exactly that. The stylesheet beside the bundle cache-busts on its own content hash through **[`Admin::css_cache_version( $css_path, $fallback )`](../includes/admin/class-admin.php)** — a SCSS-only rebuild leaves both the JS hash and the plugin version untouched, so a sheet keyed on either serves from cache behind a stale `?ver=` and only a hard refresh lands the change. The registrar calls it for you on `index.css`; call it yourself for any stylesheet you enqueue outside the registrar, as pyrobase, nuclear-gyrobase and event-logger-nodes do.

A standalone plugin dashboard gets its **own** top-level menu ([`add_menu_page`](https://developer.wordpress.org/reference/functions/add_menu_page/)) — it shouldn't squat inside the substrate's menus, which belong to Nodes' own tools: `Admin::MENU_SLUG` (`newspack-nodes`) for settings, `Admin::STATION_MENU_SLUG` (`newspack-nodes-station`) for the station that carries the Console, Overview, Jobs, Vault and the rest. If your dashboard genuinely *is* a Nodes-internal tool, register it as a `host: 'station'` tab — `registerTab( { id, label, host, component, order, slug } )` from [`@newspack-nodes/shared/tabs/tabRegistry`](../src/shared/tabs/tabRegistry.js) — rather than as an `add_submenu_page`. Either way, the gate above ([`Capabilities::can( MANAGE )`](../includes/class-capabilities.php)) keeps visibility consistent with the substrate.

This example mounts `DebugOverlay` in §9, so its stylesheet names `newspack-nodes-graph`, and WordPress loads the two handles beneath it in the cascade §6's sheet stacks: theme, then ui, then graph. A dashboard that draws no graph omits `style_deps` and takes the registrar's default, `[ 'wp-components', 'newspack-nodes-ui' ]`. A consumer wanting the palette without the component skin names `newspack-nodes-theme` alone and carries `.newspack-nodes-theme` on its root, as pyrobase's editor pages do — the tokens are defined on that class, so a root without it resolves no `var(--np-*)`. The same holds one level up: the component rules scope under `.newspack-nodes-ui`, so a host opts into the skin by carrying that class too. This page carries neither, because `DebugOverlay` supplies both on its own root whenever no ancestor is already a provider. `GraphView` deliberately has no stylesheet side-effect import: every host that renders the graph owns this dependency, keeping the graph CSS in one build asset instead of copying it into each consumer bundle.

> **← what the registrar owns.** **`Admin::enqueue_react_page( $args )`** does the ~40 lines `enqueue_insights_assets` would otherwise hold and every dashboard would repeat: read the `$_GET['page']` and bail if it's not yours; `file_exists` the bundle; `require` the `index.asset.php` manifest for deps + version; `wp_enqueue_script`; the `index.css` sidecar and its RTL companion; `wp_localize_script` the REST root + nonce as `NewspackNodesData` (which the JS transport reads). It returns the handle so a caller can layer extras. You pass it where your bundle is and which page it's for.

The `NewspackNodesData` the registrar localizes (`{ restUrl, nonce }`) is exactly what the JS [`defaultTransport()`](../src/runtime/command-transport.js) reads to authenticate the `POST /command` — so `enqueue_react_page` (PHP) and HttpOut's lazily-defaulted transport (JS) are the two ends of one wire. A `localize` key in the same args bag merges *over* those two, which is how a bundle ships page data without inventing a second global: the Topology Console arrives that way carrying its per-topology partition counts, the active topology set, and the three config-inherited defaults its settings panel offers as hints for unset frontmatter.

**Give one handle one enqueue site.** [`wp_localize_script()`](https://developer.wordpress.org/reference/functions/wp_localize_script/) concatenates onto a handle rather than replacing, so the later assignment wins whole and the earlier call's extras are gone. The substrate's own event-dashboards bundle is enqueued on both the station path and the tab-bundle path, and the tab-bundle payload is the one that survives. Either enqueue a bundle in one place, or pass the complete payload everywhere you enqueue it.
...
Each `TICK` flows from a source through the summarizer and the scorer into `scored:partition`; the Consumer tails the scored records into the digest and co-commits the snapshot. Now open **Publisher Insights** (its own top-level item) in wp-admin. The page mounts, the three Fetchers fire on the first load (one POST), and you see it: **By source** counts, the **score-ranked table** (releases items at 6, community at 4–3, exactly the Scorer's weights) with its **Draft newsletter** button, and the **Total items** card. It refreshes every 30 seconds while the tab is visible. `TICK` the sources again and the counts climb once the Consumer's next checkpoint commits the new items — the two 30-second cadences are unaligned, so allow about a minute rather than expecting the very next poll. A `request_node digest FLUSH` here takes all three cards back to zero a checkpoint later, for the reason §2 gives.
...
§8 ended on the payoff — *inspect the live graph*. From the worker's REPL that's `wp nodes cli`; **on the page itself** it's the **debug overlay**, and wiring it in is two lines. The overlay reads the page's *own* live `mountExospine` graph — the same `Core.nodes` your hook built — and renders it in the shared GraphView: every node, every edge, every counter, plus a REPL that types `connect` and `remove` at it and an inspector that invokes a node's verbs. The graph is there to be inspected, so put the inspector on the page.
...
| `sliceView()` / `registerSliceViews()` | a class file per view whose whole content is an empty-model literal |
...
| `installFakeCommandWire()` | a hand-stubbed command client per suite, which tests past the wire |

That table *is* the lesson, and it's the same one the first guide ends on, lifted to the client with one addition: **dashboards are composed node graphs, not a god view-node + god command.** You add a dashboard by composing primitives — a Timer, a Tee, Fetchers, thin view nodes, small verbs — never by building a dashboard framework. Uphold the `fill()` contract, decompose both sides, lean on the shared pieces, and the next dashboard is the handful of files you care about: the slice verbs, the view nodes, the widgets, and the hook that wires them.
...
- **[writing-a-real-dashboard.md](writing-a-real-dashboard.md)** — then this one: what a dashboard owes the Topology Console, the station and `release:archive`.
...
- **[`newspack-event-logger-nodes`](https://github.com/Automattic/newspack-event-logger-nodes/blob/main/README.md)** — the production application: four admin dashboards (Performance, Error Log, Gyroscope, Request Log) built on these same primitives, including the SSE ones this guide's poll shape deliberately doesn't cover.
