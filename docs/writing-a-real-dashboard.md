...
The toy dashboard is a closed loop: your page, your bundle, your one verb. A real dashboard leaks into surfaces the substrate owns:

![Four things you ship, each landing in surfaces the substrate owns, tagged with the section that spells out each contract. Your Service CI appears in the class catalog, whose one array feeds both the Palette and the Inspector (§1, §2). Your console surface lives inside TabHost and the tab registry, which feed the debug overlay and the station, both handing ReplFooter a measured ceiling (§3, §5). Your bundle externalizes through WP_EXTERNALS to WordPress enqueue handles and ships through release:archive (§4, §7). Your API credentials go in the Vault station tab (§6). The shipped dashboards' load bounds (§8) sit beside them.](img/wrd-shared-surfaces.png)

Each is a seam where the toy's assumptions stop holding. The sections below take them one at a time, and close on what the shipped dashboards do to stay fast under load.
...
![The catalog gate as two decision rails. Classes_CI_Node::cmd_dump asks three questions of every concrete *_Node under a registered namespace — category Hidden, category empty (no node_schema() at all, as SSE_Out_Node), or 'hidden' => true (a real category opted out, as SSE_In_Node under I/O) — and a yes on any drops the class from the catalog entirely, which leaves the Inspector's catalog.find() undefined and its Verbs section empty. CatalogProvider publishes the one array to the Palette, which keeps Service and Remote off as NON_DRAGGABLE_CATEGORIES, and to the Inspector, which renders commands as signed command buttons and requests as LOCAL request buttons. Beside them sits the browser's second catalog, built by useJsCatalog() from includeNodes and read once at first render. Beneath: what to declare (Transform or Source for a tile and verbs, Service for verbs without a tile, Hidden for neither, and 'Hidden' where 'Service' was meant as the first thing to check when a CI shows no buttons), and the per-verb flags hidden, multiple and action.](img/wrd-catalog-gate.png)

`Classes_CI_Node::cmd_dump` builds the catalog every worker-facing surface reads. It scans every registered composer classmap for concrete `*_Node` classes under a registered namespace, inlines the serializable half of each `node_schema()`, and — the line that matters — **drops a class on any of three conditions** ([`includes/rest/class-classes-ci-node.php`](../includes/rest/class-classes-ci-node.php)):
...
Any one removes the class from the catalog **entirely**. `SSE_Out_Node`, a pure HTTP response writer, never overrides `node_schema()`, so it inherits `Node::node_schema()`'s empty category; `SSE_In_Node` declares `'category' => 'I/O'` with `'hidden' => true`, because a patron node configures it programmatically.

"Entirely" is the gotcha, because that *same array* reaches two surfaces. `CatalogProvider` publishes it once as `classes`, and each surface reads it from context rather than down a prop chain ([`src/topology-console/CatalogContext.js`](../src/topology-console/CatalogContext.js)):
...
One array, two consumers: **`category: 'Hidden'` hides your class from the palette AND blanks its inspector verb buttons**, which for a Service CI is exactly backwards.

### Why a Service CI shouldn't be draggable

A Service CI like `Insights_CI` is **mounted** into every request graph (`make_node( 'Insights_CI', 'insights-demo' )` on `request_graph_ready` — [toy guide §2](writing-a-dashboard.md#2-serve-the-data--three-small-slice-verbs-not-one-god-command)) and never `make_node`'d from the canvas. You want it off the palette, where a drag would mint a stray second `insights-demo` nobody routes to, and its `counts` / `top` / `accumulated` buttons still on the inspector. Dropping it from the catalog kills both.

The substrate's answer is a **palette-only** filter, not a catalog drop. [`Palette`](../src/topology-console/components/Palette.js) keeps a `NON_DRAGGABLE_CATEGORIES` denylist — "categories the catalog carries that the palette must never offer", as its docblock puts it:
...
The gate governs the PHP catalog, which is the right one wherever `make_node` runs in a worker — a topology being edited, a remote graph driven over SSE. Where `make_node` runs in the BROWSER, the sheet's right-hand panel applies: the browser can instantiate nothing outside `CommandInterpreterNode.includeNodes`, so [`useJsCatalog()`](../src/topology-console/hooks/useJsCatalog.js) builds the same `{ shell_name, category, description, accepts_fill, has_target, fans_out, is_interpreter, arguments }` entries out of that table, derives `fans_out` and `is_interpreter` from the class the same way, and sorts on the same `[category, shell_name]` pair. Every rule above therefore has to be satisfied twice, independently, and a registration landing after first render stays invisible until the surface remounts.

### The per-command flags, one level down

The class-level gate is coarse. Inside a class that made the catalog, `strip_commands()` projects each `commands[]` entry down to `{ name, description, args }` — dropping the non-serializable `handler` and the `capability` the base gate enforces server-side — and carries forward the three flags the sheet's lower right lists: `hidden`, `multiple` and `action`. Ticking a verb in the editor writes a `command_node` line into the `.tsl`, which runs on every worker boot. Where that line addresses is `verbUsesConfig`'s decision ([`src/topology-console/utils/editorLines.js`](../src/topology-console/utils/editorLines.js)), and the file wins: a form the `.tsl` already recorded stands, because the operator wrote it, so editing an existing invocation never flips a bare address to `:config` or back. Only a verb the Inspector has just added falls to the class rule — an interpreter takes its verbs bare, every other class through its auto-wired `:config` sibling, the same split §2's dispatch table draws for a live send.
...
The toy's `Insights_CI` declares a `commands` array and gets command buttons. A real pipeline also has **runtime triggers** — the fire-and-forget `TM_REQUEST` verbs that drive the graph (the [toy's §8](writing-a-dashboard.md#8-run-it--drive-the-pipeline-watch-the-dashboard-then-inspect-the-graph) `TICK`). Those live under a *different* schema key, `requests`, and the Inspector renders them as a distinct button kind.
...
![Three stacked height budgets. The fallback, maxHeight(), subtracts 32 for the WordPress admin bar, 64 for the station header, 40 for the tab bar and 38 for the prompt bar (FIXED_CHROME_PX, 174) plus a 6-pixel handle overhang from window.innerHeight, never going under 80, and cannot see the TabHost bar the console really sits under. Host A, the Topology Console station tab, measures .topology-app through a ResizeObserver with a debounce of 0, which runs after layout and before paint, and gives the transcript appHeight − 38 − 0, never under 80, returning null before layout; it carries no header term, because the brand header belongs to the station. Host B, the overlay's Console tab, starts from the panel's live frame.h and subtracts a 64-pixel header, the 38-pixel prompt bar, a measured tab bar and a 4-pixel handle, never going under 80. Beneath: the transcript's overlay height flowing back to autofit through useGraphSurface and ChromeProvider, with useChrome() throwing outside a provider rather than defaulting the obstruction to zero; and what the two hosts share — one persisted REPL height under newspack-nodes:topology-console:repl-height, a camera stored as a delta from autofit, and ConsoleShell, useGraphSurface and useCompletion.](img/wrd-console-ceilings.png)

[`ReplFooter`](../src/topology-console/components/ReplFooter.js) is deliberately dumb about its own ceiling. It accepts a `maxHeightPx` prop and, when set, prefers it over its viewport fallback:

```js
const ceiling = maxHeightPx ?? maxHeight();
```

That fallback, `maxHeight()`, is the trap: `FIXED_CHROME_PX` sums four magic constants, so it is right only on a full-page console with exactly that chrome above it. The two hosts that mount a REPL each compute their own ceiling and pass it in.

The ceiling is the one thing they compute apart. Everything else in that surface is shared, and a third host reuses it rather than rebuilding it. Both mount the same `ConsoleShell` ([`src/topology-console/components/ConsoleShell.js`](../src/topology-console/components/ConsoleShell.js)), which owns the vertical order — Header, canvas, REPL footer — and swaps `GraphView` for a placeholder div until `ready`; it returns a bare Fragment, so it drops into each host's own `.topology-app` CSS grid, whose children claim the `canvas` and `repl` areas by class. Both build its chrome through `useGraphSurface` (`src/topology-console/hooks/useGraphSurface.js`), which composes the palette and inspector collapse flags, the transcript's overlay height and the REPL's expand state and input ref into one hook, because a behavior threaded separately has to be added twice and the overlay is the copy that gets missed. Anything belonging to one host — the console's edit toolbar, topology picker and modals, the overlay's floating-panel chrome — stays a sibling of the shell instead of growing a prop on it.
...
**Host A — the full-page Topology Console.** It measures its `.topology-app` grid with a `ResizeObserver` rather than trusting the viewport (the `appRef` measure effect in [`TopologyConsole.js`](../src/topology-console/TopologyConsole.js)):
...
[`useContainerRefit`](../src/shared/hooks/useContainerRefit.js) is the shared hook every chart and canvas observes through, and §8 covers what it does under load.
...
The handle term stays named at `0` so the ceiling reads as the sum of the chrome it accounts for.

**Host B — the debug overlay's Console tab (`InspectorTab`).** Same disease, different frame. It measures *its own* tab bar instead of hardcoding it, and answers `0` where the host renders no bar at all ([`src/debug-overlay/tabs/InspectorTab.js`](../src/debug-overlay/tabs/InspectorTab.js)):
...
`frameHeight` is `frame.h`, the panel's live height from [`useDebugFrame`](../src/debug-overlay/useDebugFrame.js), threaded down as the `frame` prop in `tabProps`. The prompt-bar term is `38` because the transcript's CSS anchor sits at `bottom: 38px` (`.topology-repl__transcript` in [`src/topology-console/styles/graph-view.scss`](../src/topology-console/styles/graph-view.scss)): get that number from the CSS, not from intuition.

The canvas stores its camera the same way, and for the same reason. A pan or zoom persists as a DELTA from autofit — `{ dcx, dcy, zoom }` through `deltaFromAutofit` / `viewportFromDelta` ([`src/topology-console/utils/autofitDelta.js`](../src/topology-console/utils/autofitDelta.js)) — never as an absolute viewBox, which goes stale the moment the window, the panel or the transcript changes the canvas size. The zero delta `{ 0, 0, 1 }` *is* autofit, so an untouched view reopens as autofit at any size, and a panned one re-derives against the new fit. Both directions refuse rather than guess: a box without positive width and height, or a stored `zoom` that is zero, negative or unreadable, returns null and leaves the caller at autofit instead of an infinite or mirrored viewBox.
...
`buildDashboards` rewrites every `@wordpress/*` import to a `window` global and records the matching enqueue handle in `*.asset.php` ([toy guide §6](writing-a-dashboard.md#6-build-it--a-few-lines-not-a-build-system)). The map that drives it is `WP_EXTERNALS` ([`src/build-kit/index.mjs`](../src/build-kit/index.mjs)). Look at what's *absent*, and at the docblock explaining why:
...
![One tab registry on window between two hosts. The floating overlay (host overlay) holds io-overview at order 0, console at 1 and event-logger-nodes' eln-current-request at 2, hands every tab publishHeader, storageKey and frame, and mounts on every page newspack_nodes/overlay_pages names. The station (host station) holds overview at 0, jobs 10, topology-console 15, partition-viewer 20, log-viewer 25, config-audit 30, vault 30, sessions 35 and aggregator 40; the Console, Vault, Sessions and Aggregator are lazy, the Console, Partition Viewer and Log Viewer are fullBleed, and the two 30s sort by label. The registry's registerTab shadows a repeated id across hosts, and getTabs returns a host's own tabs plus the both tabs, gated per read and sorted by order then label. A lower rail traces a lazy tab from the station_tab_bundles filter to the load recipe on window.NewspackNodesLazyTabs, the LAZY_TABS placeholders spreading each tabMeta.js, and first activation, where loadTabBundle injects the script and the real bundle's registerTab shadows the placeholder by id. Beneath: what TabHost does for every tab (mount only the active one, hide the bar at one tab, the WAI-ARIA tabs pattern, the landing tab resolved once at first render), how syncUrl resolves ?tab=, and why a consumer's lazy bundle never joins the bar.](img/wrd-tab-host.png)

The center of it is **`TabHost`** ([`src/shared/tabs/TabHost.js`](../src/shared/tabs/TabHost.js)) — one component, two hosts. It reads the tab registry for a given `host`, renders a tab bar (hidden when ≤1 tab), and **lazily mounts only the selected tab**, keyed on the active id so each tab's build-before-render runs fresh on switch:
...
A tab is a descriptor in the registry ([`src/shared/tabs/tabRegistry.js`](../src/shared/tabs/tabRegistry.js)). `id`, `label`, `host` and `component` are required; `order`, `slug`, `param`, `gate`, `icon` and `fullBleed` are optional:
...
- **The floating overlay** mounts tabs with `host="overlay"` — the substrate registers Overview and the Console tab from §3 ([`src/debug-overlay/tabs/index.js`](../src/debug-overlay/tabs/index.js)), and a consumer adds its own: event-logger-nodes contributes `eln-current-request`, labelled Request.
- **The full-page station** ([`src/station/Station.js`](../src/station/Station.js)) mounts `host="station"` inside a `position: fixed`, full-height admin container positioned against `useAdminMenuWidth()`, so a full-screen canvas tab gets usable height, and passes `syncUrl`, which makes the host own the page's query string. It also gates the floating overlay's REPL off the Console tab, where a second overlay REPL would collide on the shared `_output` infra.

One nuance of `syncUrl` decides whether a deep link works: until the first click retires the initial `?tab=` slug, a slug naming a tab the registry does not hold yet is retried on every registry version bump and the URL is left alone, so a slow lazy bundle, or a consumer's tab registering after the host mounted, can still be reached by link.

Absent a `?tab=` slug, the landing tab is whichever sorts first when the host first renders, and it is resolved once. Overview holds that slot only because WordPress runs same-priority callbacks in registration order, so the order-0 tab is in the registry before the station bundle executes. Swap those two `add_action` calls, or add an eager bundle at an earlier priority, and the station lands on whatever lowest-order tab happens to exist — no error, just the wrong tab.

Whichever tab wins, the host reports the RESOLVED id — never the requested one — through `onActiveTabChange`, on mount and on every switch, and `undefined` while it has no tabs at all; `Station` keys its floating overlay's `storageKey` and `buildRepl` on that value.

The station also renders **one shared header** and spreads its controls slot into every tab as `headerControlsSlot`. A tab with a toolbar of its own places it through `HeaderSlot` ([`src/shared/components/HeaderSlot.js`](../src/shared/components/HeaderSlot.js)) rather than rendering it directly, because that prop carries three states: an Element portals the controls into the header, `undefined` means no host offered a slot and the controls render inline — the standalone admin page — and `null` means the slot has not mounted yet, so the controls are withheld for that render. Render them inline on `null` and they land in the tab body, then jump to the header a frame later. `LogStreamViewer`, `SessionsAdmin`, `AggregatorStatus` and `VaultAdmin` all take the prop and hand it straight to `HeaderSlot`. Three screens — `TopologyConsole`, the Overview dashboard and `ConfigAudit` — write those three branches out by hand instead, so a grep for `createPortal` near `headerControlsSlot` finds copies rather than one authority. Follow the `HeaderSlot` import; don't copy one of the three.

The floating overlay runs the same idea through a different wire, and an `overlay`-host tab has to know which. `DebugPanel` renders one `Header` ([`src/topology-console/components/Header.js`](../src/topology-console/components/Header.js)) above the tab bar and hands every tab a `publishHeader` callback in `tabProps`; a tab with controls of its own calls it with the props `HeaderControls` accepts — the Console tab publishes its cwd cluster, `pathOptions`, `path` and `onPathChange` — and calls it with `null` on unmount to retract them. The panel spreads that object *after* its own `mode` and `onClose`, so a tab publishing either key takes over the panel's mode or its close button. `onClose` is itself the switch that collapses the console's NEW/OPEN/EDIT/LIVE row into a lone close X, which is why the overlay's header shows no mode buttons. Two more props ride along in `tabProps`: `storageKey`, the per-tab canvas-layout key, and `frame`, the panel's live `{ x, y, w, h }` geometry, which §3's ceiling is computed from. A tab needing neither ignores both, as event-logger-nodes' Request tab does.

### Registering a tab, and shipping it lazily

You register a tab by filtering into **[`newspack_nodes/station_tab_bundles`](API.md#filters)** (the PHP analogue of the toy's CI mount); the sheet's lower rail shows each entry's shape and what `Admin::enqueue_station_tab_bundles()` skips. Importing the bundle is what registers the tab, so PHP never names one.
...
`src/vault/tabs.js` spreads that same object with its component, and `station/lazyTabs.js` spreads it into the placeholder, so the arriving bundle's own `registerTab` shadows the placeholder by id. Two spellings of the descriptor would rename, reorder or duplicate the tab the moment the bundle lands.

**`lazy` is substrate-internal, and a consumer that reaches for it gets silence**, as the sheet's red box traces: the `LAZY_TABS` array names its four handles by hand with no filter to extend it, and `loadTabBundle()` sits beside it in `src/station/`, which [`alias-map.cjs`](../src/build-kit/alias-map.cjs) never reaches — it maps `runtime`, `debug-overlay` and `shared` and nothing else. Event-logger-nodes' Request tab is the shape a consumer ships: an eager bundle, no `lazy` key.
...
The most production-real question — **where do the API credentials go?** — has an answer you *don't* write. The substrate already ships it: the **Vault**, a built-in station tab at `admin.php?page=newspack-nodes-station&tab=vault`, not a WordPress Settings-API page and not a React tree you have to author.
...
**A shared-source edit fans out to every consumer bundle.** The `@newspack-nodes/*` surface is three aliases, all resolved from one base by [`src/build-kit/alias-map.cjs`](../src/build-kit/alias-map.cjs):
...
|---|---|---|
| [`errorMessage`](../src/shared/errorMessage.js) | `errorMessage` | The readable text behind a TM_ERROR reply, including the wording for a payload carrying nothing. |
| [`rateSmoother`](../src/shared/rateSmoother.js) | `RateSmoother` | Windowed average plus EMA behind every per-second readout, because a raw delta between two samples reads as a burst or a zero. |
| [`theme`](../src/shared/theme.js) | `applySkin`, `initSkin`, `resetSkin`, `getStoredTheme`, `isValidTheme`, `THEMES`, `DEFAULT_THEME`, `THEME_STORAGE_KEY`, `SKIN_EVENT` | Skin storage and application. One `theme-<slug>` class on `<html>` is the whole skin state, so no two surfaces on a page can disagree — and anything holding a token's COMPUTED value rather than inheriting it needs `SKIN_EVENT` to redraw — event-logger-nodes' `ThemedRoot` repaints the wp-admin gutters on it; a chart needs nothing, because `chartColor` hands it the token itself, as §8 covers. |
| [`components/AreaTimeChart`](../src/shared/components/AreaTimeChart.js) | `AreaTimeChart` (default) | The one area-chart frame every dashboard time chart draws on: axes, stacked or overlaid bands, the legend beside the plot, the tooltip, and a stack toggle in the corner the reader flips per chart. Hand it series sharing one slot list, a `yFormatFor( peak )` factory, a `colorAt( label, index )` and a `yLabel` naming the quantity, whose unit the ticks carry; the Topics panels and event-logger-nodes' Aggregate and Category charts are its callers. |
| [`components/AskPageRing`](../src/shared/components/AskPageRing.js) | `AskPageRing` (default) | The ring around a target the size of the page, mounted for as long as the picker is armed. An outline there is painted over by the box's own children, so it is an overlay, and it goes through `ModalPortal` because the shell it rings is a stacking context of its own. It measures `[data-ask-page]` itself — `clientWidth`/`clientHeight` exclude that box's scrollbar — and re-measures on a resize and when a menu fold settles. |
| [`components/ChartLegend`](../src/shared/components/ChartLegend.js) | `ChartLegend` (default) | The legend beside every dashboard chart: one native button per series, scrolling inside the chart's height; a click picks that series alone and a ctrl- or cmd-click adds to the pick, through `useSeriesSelection`. |
| [`components/ColumnPicker`](../src/shared/components/ColumnPicker.js) | `ColumnPicker` (default) | The checkbox row a "Cols" toolbar button reveals; `useColumnPicker` owns the selection it edits. |
| [`components/ConnectionBanner`](../src/shared/components/ConnectionBanner.js) | `ConnectionBanner` (default) | The polite live-region banner a dashboard renders while its transport is down. |
| [`components/HeaderSlot`](../src/shared/components/HeaderSlot.js) | `HeaderSlot` | §5's placement for a dashboard's own controls: portalled into the host's shared header, or rendered inline where no host offers a slot. |
| [`components/LogBrowser`](../src/shared/components/LogBrowser.js) | `LogBrowser` (default) | The Live/Replay browse rail; both log-stream dashboards drive it as a segment browser. |
| [`components/LogListHeader`](../src/shared/components/LogListHeader.js) | `LogListHeader` (default) | The column-header row capping a log pane, sized from the same cell classes as the rows. Render it as the sibling immediately before `LogRowList`'s `role="rowgroup"` element: an adjacent-sibling rule paints the seam, so a wrapper element around either one breaks it silently. |
| [`components/LogRowList`](../src/shared/components/LogRowList.js) | `LogRowList` (default), `DEBUG_MAX_ROWS` | §8's ring-aware virtualized row list, plus the cap on the un-virtualized debug regime. |
| [`components/LogStreamViewer`](../src/shared/components/LogStreamViewer.js) | `LogStreamViewer` (default), `debugValue` | The chrome every log-stream dashboard wears: toolbar, reconnect banner, browse rail and row list. |
| [`components/Modal`](../src/shared/components/Modal.js) | `Modal` (default) | The plain-DOM dialog shell carrying the canonical `.newspack-nodes-modal` role, dismissed through `useDismissable`. Its `role="dialog"` is hardcoded, so a message the operator must acknowledge before carrying on builds its own shell around `role="alertdialog"` and reuses the class and the hook directly — `AlertModal`, the Overview board's mutation-refusal box, is the worked example. |
| [`tabs/TabHost`](../src/shared/tabs/TabHost.js) | `TabHost` (default) | §5's one tab host, behind both the floating overlay and the full-page station. |
| [`tabs/tabRegistry`](../src/shared/tabs/tabRegistry.js) | `registerTab`, `getTabs`, `getTabsVersion`, `subscribeTabs`, `resetTabs` | §5's registry on `window`: a bundle registers at import time, the host reads and subscribes, and a test resets. |
| [`helpers/addSliceFetcher`](../src/shared/helpers/addSliceFetcher.js) | `addSliceFetcher` | One dashboard slice wired in one call, with the Fetcher connected last (§8). |
| [`helpers/controlMsg`](../src/shared/helpers/controlMsg.js) | `controlMsg`, `isControl` | The one minter of a view node's control message, and the recognizer that admits what it mints. |
| [`helpers/egressPath`](../src/shared/helpers/egressPath.js) | `egressPath` | The TO path a browser-minted command travels: the observe-only `_shell` Tap, the `_http` egress, then the server CI. |
| [`hooks/useAdminMenuWidth`](../src/shared/hooks/useAdminMenuWidth.js) | `useAdminMenuWidth` (default) | The WordPress admin menu's width as a number, so a fixed-position UI sits flush against a menu that folds. |
| [`hooks/useAskPicker`](../src/shared/hooks/useAskPicker.js) | `useAskPicker`, `ASK_TRIGGER_ATTR`, `ASK_PAGE_ATTR`, `ASKING_CLASS` | Ask-about-this-element picking off one `data-ask` attribute; the attribute and class are exported because the shared SCSS and a consumer's trigger button name them too. |
| [`hooks/useBatchedPoll`](../src/shared/hooks/useBatchedPoll.js) | `useBatchedPoll`, `useCatalogSlice` | §8's poll backbone — exospine mount, fan-out Tee, hitchhiking Timer, visibility gate — and the catalog slice whose next tick is its retry. |
| [`hooks/useColumnPicker`](../src/shared/hooks/useColumnPicker.js) | `useColumnPicker`, `gridTemplate` | A table's persisted visible-column set, and the CSS grid track list that lays it out. |
| [`hooks/useCommandOnce`](../src/shared/hooks/useCommandOnce.js) | `useCommandOnce` | One verb sent exactly once per call on the batched tick, its reply landing on the hook's own result node. |
| [`hooks/useContainerRefit`](../src/shared/hooks/useContainerRefit.js) | `useContainerRefit` | The resize hook of §3 and §8: debounced by default, `0` for a measurement, and deaf to the observation for the box it just drew. |
| [`hooks/useDeepLinkedSelection`](../src/shared/hooks/useDeepLinkedSelection.js) | `useDeepLinkedSelection` (default) | A picker's `?param=` contract: seed the selection once from the URL, then reflect every user pick back into it. |
| [`hooks/useDismissable`](../src/shared/hooks/useDismissable.js) | `useDismissable` | ESC and mousedown-outside, the two ways every dialog closes besides its own button. The listeners sit on `document`, one pair per mounted caller and no topmost check, so ESC closes every open dialog at once; a dialog that must shield the one behind it adds its own gate. |
| [`hooks/useLogPositions`](../src/shared/hooks/useLogPositions.js) | `useLogPositions` (default), `useLogStatusSegments`, `useSegmentBrowse`, `segmentPositions`, `replayPositions`, `stepPosition` | The browse model both log-stream dashboards share, expressed as the SSE `positions` seed their stream URL already carries. |
| [`hooks/usePageVisibility`](../src/shared/hooks/usePageVisibility.js) | `usePageVisibility` (default) | The one read of tab visibility every substrate poller gates on. |
| [`hooks/usePersistedState`](../src/shared/hooks/usePersistedState.js) | `usePersistedState`, `usePersistedChoice` | A preference that outlives the page: read the key, validate what came back, fall back, write it again. |
| [`hooks/useRouterTick`](../src/shared/hooks/useRouterTick.js) | `useRouterTick` (default) | "Call me on the router heartbeat", for any React poller — a passenger on a backbone another mount owns. On a page where the passengers are the only graph traffic, the first one raises the backbone and the last one out removes it, so a lone poller needs no owning mount beside it. |
| [`hooks/useSeriesSelection`](../src/shared/hooks/useSeriesSelection.js) | `useSeriesSelection`, `useLegend` | Which of a chart's series the reader picked: a set of labels, empty meaning all, read against the labels the chart carries now so a retired series drops out of the effective set on its own. `useLegend( series, colorAt )` is the whole legend on top of it — the rows coloured by rank over the full list, the `drawn` series each carrying its colour, and the pick — which is what a chart drawing one series per label wants; a chart filtering points by class takes `useSeriesSelection` alone. |
| [`hooks/useStreamGraph`](../src/shared/hooks/useStreamGraph.js) | `useStreamGraph`, `useSteppedRead`, `useLogCatalog` | A streaming dashboard's whole graph (link, stream Tee, view), the one-record stepped read, and the polled subscription catalog. |
| [`hooks/useTimeChart`](../src/shared/hooks/useTimeChart.js) | `useTimeChart`, `openFrame`, `drawAxes`, `setupTooltip`, `buildTimeSlots`, `formatXTick`, `MARGIN`, `PALETTE`, `chartColor`, `BUCKET_MS`, `BUCKET_SECONDS`, `BUCKET_MINUTES`, `NUM_BUCKETS`, `DEFAULT_RETENTION_SECONDS` | The one d3 frame every dashboard time chart is drawn on; a caller owns its marks and nothing else. The legend is `ChartLegend`, beside the SVG in the `.newspack-nodes-chart__row`, not a group inside it — and `AreaTimeChart` is the frame already drawn, which a new chart should take before reaching for this. |
| [`hooks/useVirtualization`](../src/shared/hooks/useVirtualization.js) | `useVirtualization` (default) | Row-window math for a long list, measured against whichever element actually scrolls: `'self'` for the list's own box, an ancestor selector, or `null` for the window, which also skips the update while the list is off screen. A selector matching no ancestor throws on the first measurement rather than degrading, so a list virtualizing against `.components-modal__content` must really be inside a Modal. Its scroll state lands in React, which is why `LogRowList` reads geometry live instead. |
| [`nodes/catalog-list-view-node`](../src/shared/nodes/catalog-list-view-node.js) | `CatalogListViewNode` | The slice view holding a picker's catalog; `useLogCatalog` is its only builder. |
| [`nodes/command-result-node`](../src/shared/nodes/command-result-node.js) | `CommandResultNode` | Where a one-shot command's reply lands. Every reply publishes and notifies, refusals included — the opposite of a slice. |
| [`nodes/log-stream-view-node`](../src/shared/nodes/log-stream-view-node.js) | `LogStreamViewNode` | The view-node base under every log-stream dashboard: the 100 000-row ring, the paused belt and step budget, the decaying rate readout, seek tracking and the shared control verbs. |
| [`nodes/seekTracker`](../src/shared/nodes/seekTracker.js) | `SeekTracker`, `browseControl`, `LIVE`, `REPLAY` | Node-side seek and position tracking, derived from each record's `segment:offset:length` breadcrumb. |
| [`nodes/slice-view-node`](../src/shared/nodes/slice-view-node.js) | `SliceViewNode`, `sliceView`, `registerSliceViews` | The per-widget view-node base a dashboard's slices extend, its declarative factory, and the name registration a TSL or palette surface needs ([ADR-16](architecture-decisions.md#adr-16-js-node-class-resolution--names-are-the-tsl-surface-classes-are-the-api)). |
| [`test-utils/fakeCommandWire`](../src/shared/test-utils/fakeCommandWire.js) | `installFakeCommandWire`, `makeFakeCommandWire`, `answerBatch`, `commandReply` | A `/command` server double whose seam is the wire, so pack/unpack, HttpOut, the Router and the interpreter all run for real. |
| [`test-utils/fastClock`](../src/shared/test-utils/fastClock.js) | `runClockFast` | The substrate clock run faster than the wall clock, so a test reaches the next poll boundary instead of waiting it out. |
| [`utils/answerStatus`](../src/shared/utils/answerStatus.js) | `answerStatus` | An answer as the line a row or form shows for it: working, failed with this text, or succeeded. |
| [`utils/axis-ticks`](../src/shared/utils/axis-ticks.js) | `axisDuration`, `binaryTicks`, `integerTicks` | Ticks and units for a value axis, so a byte axis ticked in base 10 never reads "977 KB". |
| [`utils/buttonClass`](../src/shared/utils/buttonClass.js) | `primaryButtonClass` | The one composition of a confirm button's class list, disabled state included. |
| [`utils/fnv1a`](../src/shared/utils/fnv1a.js) | `fnv1a` (default) | The 12-character URL identity this port shares with PHP's `Log_Manager::url_hash()`; the two agree on ASCII. |
| [`utils/formatters`](../src/shared/utils/formatters.js) | `formatBytes`, `formatByteRate`, `formatMsgRate`, `formatCount`, `formatAge`, `formatEta`, `formatEtaSeconds`, `etaSeconds`, `compactFixed` | The presentation formatters, so no two surfaces disagree about what 881869 bytes reads as. |
| [`utils/formatUtils`](../src/shared/utils/formatUtils.js) | `formatDuration`, `formatLocalDateTime`, `formatTime`, `getDurationClass`, `getDurationColor`, `getStateColor`, `getStatusCategory`, `getStatusClass`, `getStatusColor`, `getTextColor`, `hexToRgba`, `STATUS_COLORS` | The shared color and readout vocabulary. Every color is a literal hex and skin-independent, because these paint fills; the text colors that follow the theme live in the SCSS tokens. |
| [`utils/parseOffsetJump`](../src/shared/utils/parseOffsetJump.js) | `parseOffsetJump` (default) | The grammar behind the Jump box, whose three-part form is a message ID verbatim and whose bare-offset form resolves against the caller's current segment. With no segment to resolve against it returns null rather than assuming segment 0, and unparseable text is the same null, because the box runs this on every Enter. |
| [`utils/queryParams`](../src/shared/utils/queryParams.js) | `getQueryParam`, `setQueryParam` | The `?param=` deep-link surface, written so setting one param preserves every other. |
| [`utils/storage`](../src/shared/utils/storage.js) | `readStorage`, `writeStorage` | `localStorage` without the try/catch at every call site; both absorb the failure and leave the caller on its default. |
...
![Every bound on the data path, in order. Before any request leaves the page: the 1000 ms poll floor that throws a TypeError, usePageVisibility (which also closes an SSE stream and returns its php-fpm slot), paused and enabled: false, the 30000 ms CATALOG_MS, and useLogTailStream's history or follow seek. At the view node: PUBLISH_THROTTLE_MS 500, MAX_SAMPLES 5,761 over 86,400 seconds, ENTRY_TTL_MS 300,000 measured by arrival, GAP_INTERVALS 6, the 100,000-row ring filtered at ingest, and the 1,000 and 262,144-character clips; SettingsAuditViewNode keeps 5,000 entries on the same throttle. At the row list: O(rows on screen) per frame, glide hysteresis between 1 and 2 rows, a 300-row glide debt, 250 ms stats, and one requestAnimationFrame per reorder move. At the chart: 1,000 points for an 1,800-pixel panel, bucket alignment and busiest-first ranking, hold-and-last gauges against zero-and-rate rates, memo, useDeferredValue and a twelve-row tooltip, the two re-skin moves, and a 150 ms useContainerRefit debounce. Beneath: which metrics dedup across co-readers, which sum, and which judge freshness by the sample's own timestamp; and the two orders that are contracts, the Fetcher connecting last and the overlay's one 50 ms setInterval.](img/wrd-load-bounds.png)

**A poll cadence has a floor of 1000 ms, and it is a `TypeError` below it.** [`useBatchedPoll`](../src/shared/hooks/useBatchedPoll.js) enforces it because the batch **is** the lock/flush bracket the Router puts around `notifyTimer`, and only a timer hitchhiking the router tick sits inside it. Above 1000 ms the timer throttles against the shared wall-clock grid ([ADR-17](architecture-decisions.md#adr-17-timers-fire-on-a-shared-wall-clock-grid)), so two surfaces on one cadence meet on the same tick and share the POST. Sub-second work belongs to `useRouterTick`, a passenger on somebody else's backbone. `useOverviewStats`, behind the debug overlay's Overview cards, takes neither route, for the reason the sheet's last box gives and because the overlay also runs on pages that mount no graph at all.

**A surface nobody is looking at costs nothing.** The sheet's top row orders the three gates by thrift. The three catalog hooks (`useClassCatalog`, `useTopologyList`, `useVaults` in [`src/topology-console/hooks/useCatalogs.js`](../src/topology-console/hooks/useCatalogs.js)) all default to `enabled: false`, so every caller opts a catalog in deliberately: the console holds its classes and vaults open, hands the OPEN dialog's topology list `enabled: openModalShown`, and the overlay's Console tab enables its class catalog only once it has a cwd.

The same gate is worth more on a **streaming** dashboard, where it frees a scarce server resource rather than a request: an SSE stream occupies one php-fpm child for its whole life, so the substrate caps concurrent streams in a slot pool, and [`useStreamGraph`](../src/shared/hooks/useStreamGraph.js) opens the stream only while the tab is visible and the user has not paused. Pause goes down the same close path as the visibility gate, so it returns the slot too. [sse-host-budget.md](sse-host-budget.md) carries the arithmetic behind the bounds.

A dashboard whose subscription is *declared* — the two probe charts, the Config Audit timeline — reaches `useStreamGraph` through [`useLogTailStream`](../src/event-dashboards/hooks/useLogTailStream.js), where one `name` derives all three node names and `mode` picks the opening seek the sheet describes. A dashboard that lets the operator *pick* a log from a catalog builds its graph through `useLogReaderGraph` instead.
...
Reading the model that base publishes has a rule of its own, drawn in the sheet's lower left: dedup what the source owns, sum what the reader owns, and judge a value shown as current by its own timestamp, not by `ENTRY_TTL_MS`. `probe24hTotals` integrates each source over the union of its readers' windows, oldest end first, crediting a later window only for the fraction that sticks out past what is already covered.

**Filter at ingest, not at render.** [`LogStreamViewNode`](../src/shared/nodes/log-stream-view-node.js) runs `matchesFilter()` before a row enters the ring, and `_trackPosition()` calls `_publish()` only where `SeekTracker.track()` reports a segment change or a replay boundary, so a stream at full rate never sets state once per record to keep the rail highlight current.

**Per-frame work is bounded by the viewport, not by the input rate.** [`LogRowList`](../src/shared/components/LogRowList.js) carries the bounds in the sheet's middle column, and reads its window from scroll geometry live inside the frame rather than from `useVirtualization`, whose deferred scroll-state re-render lands a frame behind the pull it would feed.

**A reorder drag writes `transform`, and touches React once**, through the slot arithmetic in [`src/event-dashboards/overviewOrder.js`](../src/event-dashboards/overviewOrder.js): `dragGapTransforms` for the per-row `translateY` each frame writes, and `dragReorder` for the order pointer-up commits. One implementation of that arithmetic keeps the gap the operator saw and the order that lands from disagreeing.

**Downsample to the pixel budget before d3 sees it.** [`TopicsChart`](../src/event-dashboards/TopicsChart.js) hands `buildAlignedSeries` a `MAX_POINTS` of 1000 and is wrapped in `memo`, so callers hand it props stable across renders — a memoized `series`, module-level formatters, the shared `fillModeForMetric` constants — and a panel whose own inputs didn't move skips the draw. Overview and Jobs hand the rollup a `useDeferredValue` copy of its input. The twelve-row tooltip drops only from the tooltip; every area and every legend entry is still drawn.

Two pure steps stand between the probe model and that cap. `topicChartSeries` ([`src/event-dashboards/topicProbeSeries.js`](../src/event-dashboards/topicProbeSeries.js)) groups the stream's entries by a key the caller supplies — Overview groups `topicprobe:view` consumers by source, Jobs groups `jobstats:view` entries by handler for its rate panels and by job key for latency — sums the metric across every identity sharing one `ts`, and takes the WIDEST of their weights rather than the sum, since identities on one instant are readers of the same window. An entry whose key comes back empty is dropped rather than collected under `''`, so a `keyOf` that can return nothing loses those identities from the panel. `buildAlignedSeries` then floors and ranks as the sheet shows.

How an empty bucket reads belongs to the metric, and `fillModeForMetric` is the whole table: `backlog` and `cacheSize` are LEVEL gauges, `{ fill: 'hold', agg: 'last' }`, and everything else falls back to RATE, `{ fill: 'zero', agg: 'rate' }`. A new rate metric therefore needs no entry, while a new GAUGE that skips one has every gap in its line drawn to zero. `queueLatencyMs` is weighted by `runsDelta` rather than `elapsed` because it is a per-run mean: weighting a mean by seconds would treat a busy window and an idle one as equals, and a per-run mean sums only within one identity — which is why Jobs groups that one panel by job key rather than by handler.

**A d3 chart re-skins only through CSS.** Applying a skin swaps one class on `<html>` and touches no React state, so a chart that read a token's computed value keeps that colour until something else happens to redraw it, and pays a second draw on mount to read it at all. `TopicsChart` reads nothing: `chartColor( index )` ([`src/shared/hooks/useTimeChart.js`](../src/shared/hooks/useTimeChart.js)) names the rank's `--chart-N` token with the `PALETTE` entry as its fallback, and that value is what the legend swatch and the area's `.style( 'fill', … )` carry, so the browser re-resolves it with the page. Set it through `.style()`, never a presentation attribute, which does not resolve `var()`; and outside a skinned subtree every token is blank, so the chart draws in `PALETTE` with no other symptom.

**Measure the container, debounced, and ignore your own box.** [`useContainerRefit( ref, callback, deps, debounceMs = 150 )`](../src/shared/hooks/useContainerRefit.js) is what every chart and canvas resizes through, and it falls back to a `window` listener where `ResizeObserver` is missing. Pass `0` when you want a measurement (§3's ceiling); leave the default when you want a redraw.
...
- **[`newspack-event-logger-nodes`](https://github.com/Automattic/newspack-event-logger-nodes/blob/main/README.md)** — the production application: four dashboards on these surfaces (Performance, Errors, Gyroscope, Request Log), an `overlay`-host Request tab registered into the same window registry, and the second consumer that proves the shared-alias rebuild rule.
