...
| [`components/AreaTimeChart`](../src/shared/components/AreaTimeChart.js) | `AreaTimeChart` (default) | The one area-chart frame every dashboard time chart draws on: axes, stacked or overlaid bands, the legend beside the plot, the tooltip, and a stack toggle in the corner the reader flips per chart. Hand it series sharing one slot list, a `yFormatFor( peak )` factory and a `colorAt( label, index )`; the Topics panels and event-logger-nodes' Aggregate and Category charts are its callers. |
