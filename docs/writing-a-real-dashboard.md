...
| [`hooks/useTimeChart`](../src/shared/hooks/useTimeChart.js) | `useTimeChart`, `openFrame`, `drawAxes`, `drawLegend`, `setupTooltip`, `buildTimeSlots`, `formatXTick`, `MARGIN`, `PALETTE`, `BUCKET_MS`, `BUCKET_SECONDS`, `BUCKET_MINUTES`, `NUM_BUCKETS`, `DEFAULT_RETENTION_SECONDS` | The one d3 frame every dashboard time chart is drawn on; a caller owns its marks and nothing else. |
...
**A d3 chart does not re-skin by itself.** Applying a skin swaps one class on `<html>` and touches no React state, so a chart keeps the colors it resolved when it drew. `TopicsChart` makes the sheet's two moves. It resolves `--chart-1` through `--chart-8` through `resolveChartPalette` on every pass, off the themed element it owns: a read taken outside the skinned subtree finds nothing and silently drops the chart to the static fallback palette, and one blank token forfeits all eight so no legend mixes two palettes. And it calls `useThemeToken()` ([`src/event-dashboards/useThemeToken.js`](../src/event-dashboards/useThemeToken.js)) to put the live slug in the redraw callback's dependency list. Any new chart or canvas that resolves a theme token to a computed value needs both, or it keeps the previous skin's colors until an unrelated render happens to repaint it.
