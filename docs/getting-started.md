...
- **memcache — optional to boot, load-bearing across processes and hosts.** The runtime comes up without it: `Bootstrap::init_memcached()` leaves the shared handle null when no servers are configured rather than failing. With usable APCu, SSE slots remain available through APCu inside one web pool, along with other `shared_first()` surfaces. That fallback is per host and is not shared with a CLI or worker process whose APCu segment is separate or disabled. HMAC command-auth still refuses wire-arrived commands because its single-use nonce/session state is deliberately Memcached-only, and cross-process live stats/tables can be absent or fragmented. Workers still spawn and drain; for the complete remote-command and live-dashboard surface, point `memcache_servers` at a running Memcached service.
