...
- **Cache backend — APCu supports one web cache domain; Memcached spans domains.** The runtime comes up without Memcached: `shared_first()` prefers configured Memcached and otherwise selects usable APCu. Normal workers are long-running REST-spawned web requests, so they share the web server's APCu domain with browser/hub command-auth endpoints. An attached `wp nodes cli` need not see that APCu: it signs commands with the site secret and transports them to the worker over filesystem IPC, where the web worker verifies them and records nonce claims through its own backend. The initial lifetime of a debugging-only direct `wp nodes run` is different because it runs in WP-CLI; without Memcached, it needs CLI APCu enabled until its REST-spawned successor takes over. Another host or independent PHP-FPM/APCu pool requires Memcached. With neither backend, or on a command-session miss, command verification fails closed.
